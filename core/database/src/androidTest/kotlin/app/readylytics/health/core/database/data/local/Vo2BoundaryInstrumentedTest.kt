...
class Vo2BoundaryInstrumentedTest {
    ...
    @Test
    fun getLatestInRangeExcludesNextMidnightAndPrefersHigherIdOnTie() =
        runBlocking {
    ...
    @Test
    fun getLatestInRangeHandlesBerlin23HourDstTransitionDay() =
        runBlocking {
            // 2026-03-29: European summer time transition (23-hour day)
    ...
    @Test
    fun getLatestInRangeHandlesBerlin25HourDstTransitionDay() =
        runBlocking {
            // 2026-10-25: European winter time transition (25-hour day)
    ...
    @Test
    fun getLatestInRangeEnforcesExactElapsed30DayLowerBoundary() =
        runBlocking {
            val targetDate = LocalDate.of(2026, 6, 15)
            val nextMidnight = targetDate.plusDays(1).atStartOfDay(berlin).toInstant().toEpochMilli()
            val thirtyDaysMs = TimeUnit.DAYS.toMillis(30)
            ...
            // When only record before lower boundary exists, returns null

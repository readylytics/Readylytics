...
interface HealthIngestionStore {
    ...
    /**
     * R2-ARCH-002/WP-16: resolves the calendar dates one already-persisted HC record with
     * [hcRecordId] currently occupies, before it is deleted and replaced by an upsert (or removed
     * outright by a deletion). Mirrors the deleted-record date lookup the Changes API path needs
     * per record, since it processes one change at a time rather than a whole window.
     */
    ...
    /** R2-ARCH-002/WP-16: deletes the local row(s) owned by one HC record, by type. */
    suspend fun deleteRecord(type: HealthDataType, hcRecordId: String)

    /**
     * R2-ARCH-002/WP-16: sleep and workout session spans whose [SleepSessionInput]/[WorkoutInput]
     * time range overlaps `[startMs, endMs]`, for `SessionLinkSweep`-based session tagging of a
     * changes-path record (or page of records) at write time (HC-004).
     */
    suspend fun sessionSpansOverlapping(startMs: Long, endMs: Long): SessionSpans

    /**
     * R2-ARCH-002/WP-16: already-persisted heart-rate samples of [recordType] in
     * `[startMs, endMs]`, mapped to the domain sample shape. Used by the Changes API path to
     * compute provisional workout TRIMP/zones/avgHr from already-stored HR at upsert time (HC-004)
     * without needing direct `HeartRateDao` access.
     */

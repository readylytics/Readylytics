...
class AppLogTest {
    ...
    @Test
    fun testLazyEvaluationWhenNotLoggable() {
        class NonLoggableSink : DomainLogSink {
            var logCalled = false
        ...
        val sink = NonLoggableSink()
        DomainLogger.installSink(sink)

        var msgEvaluated = false
        DomainLogger.scoped(tag = "TestTag", correlationId = "test-session") {
            info {
                msgEvaluated = true

...
- **Documentation Synchronization Rule:** Any PR that changes scoring formulas, thresholds, coefficients, the phase/confidence model, the profile set, the HRV-display baseline, or onboarding/score-explanation copy MUST update `ABOUT.md`, the relevant `docs/DATA_FLOW.md` section(s), and the in-app About strings (`about_*`/`tooltip_*` in `app/src/main/res/values/strings.xml`) in the same PR.
- **Documentation Review Checklist:** Before approving such a PR, confirm: (1) the implementation matches `ABOUT.md`; (2) the in-app About page, tooltips, and onboarding scoring explanations agree with `ABOUT.md`; (3) the documentation drift/presence tests (`domain/scoring/**DocumentationDriftTest*`) pass.

...
7. **Heart-rate history older than 90 days is stored as a compact per-minute summary (min/max/average plus a five-point percentile sketch) rather than every raw sample.** With the same inputs, settings, and storage tier, a historical replay is repeatable in the stored scoring timezone. Heart-rate history that has moved from raw samples to minute summaries is a measured approximation, so raw-to-minute-summary recomputation can differ slightly; replays are repeatable within the same storage tier. Each minute is read from either the raw samples or its summary, never both, so nothing is double-counted; and when a session is corrected the summaries are re-derived from the stored per-device record. Summaries created before that record existed cannot be re-derived at all — they stay marked approximate and are left untouched. No in-app action repairs them today: "Resync Health Connect data" does not replace them, so that part of your history stays approximate.

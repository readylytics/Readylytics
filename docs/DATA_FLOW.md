...
| `PeriodicHealthSyncWorker` | `workers/PeriodicHealthSyncWorker.kt` | "Background Sync" toggle in Settings. `@HiltWorker` periodic foreground service calling `HealthSyncUseCase.sync(windowDays = 2)` (shares `syncMutex` with the other two flows), bridges progress to `ForegroundSyncController`, shows/dismisses a silent transient notification (`SyncNotifications.BACKGROUND_SYNC_CHANNEL_ID`); `Result.retry()` on failure (covers `RateLimitingException`). Requires `READ_HEALTH_DATA_IN_BACKGROUND`. Uses a plain windowed read (no Changes Tokens yet — see note below). |

...
- **Idempotency (non-negotiable):** Ingestion is upsert keyed by stable HC record `id` (overlap → replace, never duplicate). Deletions in Health Connect converge via chunk-scoped ID set reconciliation without blanket `deleteAll()`. Only `clearFrozenBaselines(range)` is mutated up front, recomputed in the same walk-forward pass. A killed/failed worker must leave prior valid data intact and a retry must re-run the same range idempotently. Idempotent within a tier — a warm-tier reconstruction is a measured approximation of the original raw samples, not a bit-identical replay; see `internal-docs/DATA_FLOW.md`'s Determinism across tiers note.
...
- **File Structure:** Target ≤ 400 lines/file, hard limit ≤ 800 lines (refactor if exceeded). Settings paths map to `feature/settings/src/main/kotlin/app/readylytics/health/feature/settings/{physiologyprofile,backup,common,data}/` for grouped screens, plus a flat set of files (`SleepSettingsViewModel.kt`, `SyncSettingsViewModel.kt`, `ThresholdSettingsViewModel.kt`, `UISettingsViewModel.kt`, `HeartRateZonesViewModel.kt`, etc.) directly under `feature/settings/`.
...
- **GitHub issue templates and their email mirrors must stay in sync.** `.github/ISSUE_TEMPLATE/bug_report.md` and `feature_request.md` define the section structure that the in-app email flow mirrors. Any edit to a section header or prompt in either `.md` file MUST be mirrored in the corresponding `report_email_bug_template` / `report_email_feature_template` strings in `app/src/main/res/values/strings.xml` (and vice versa) in the same change.

...
| `core/model/src/main/kotlin/app/readylytics/health/domain/model/BodyCompositionAssessment.kt` | Domain — canonical BMI/Body Fat classification      | BMI 18.5/25/30 bands; body fat continuous male (2/6/14/18/25) and female (10/14/21/25/32) bands; fixed 10–30 reference (midpoint 20) for other/unset gender; profile midpoints shift only the gauge reference marker, never the status |
...
| `core/model/src/main/kotlin/app/readylytics/health/domain/service/HealthMetricsService.kt`     | Domain — facade (delegates)                         | `assessBmi()`/`assessBodyFatPercent()` → `BodyCompositionAssessment`                     |
| `core/scoring/src/main/kotlin/app/readylytics/health/domain/calculation/HealthMetricsCalculator.kt` | Domain — facade (delegates)                     | `assessBmi()`/`assessBodyFatPercent()` → `BodyCompositionAssessment`                     |

...
alias), so the join is a rowid seek and needs no added index. Consumers:
`HeartRateRepositoryImpl`, `ScoringHistoryRepositoryImpl`, `ScoringHeartRateDataLoader` and
`SessionLinkReconcilerImpl.recomputeWorkouts`. Enforced by
...
authorized complete interval refresh can repair them. The same pass is how a Health Connect

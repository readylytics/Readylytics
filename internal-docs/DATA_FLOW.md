...
| `LocalBackupWorker`           | `app/src/main/kotlin/app/readylytics/health/workers/LocalBackupWorker.kt`                | Before resolving its lazy Room-backed `LocalBackupManager`, it requires `DatabaseReadiness.Ready`; otherwise it retries without opening Room. Ready runs preserve the existing local encrypted backup result mapping. Backups stage into private cache (`cacheDir/backup-staging/`) under an operation UUID; publication read-back and inventory validation using `RestoreInventoryValidator` are completed before pruning existing archives from the active store, retaining prior recovery points on failure. Orphan plaintext staging is cleared on startup by `CachePrune` / `DatabaseReadyStartupInitializer`. |

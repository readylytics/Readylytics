...
| `HistoricalRunIdentity`       | `core/model/src/main/kotlin/app/readylytics/health/core/model/domain/sync/HistoricalRunIdentity.kt`                        | Immutable domain model (protocolVersion = 2) binding a historical run to its input environment: `runId`, `mode` (`FULL_INGEST` / `RECOMPUTE_ONLY`), `startEpochDay`, `endEpochDayInclusive`, `zoneId`, `startedAtEpochMs`, `sourceSelectionId` (SHA-256 of sorted device selections), `algorithmRevision`, `scoringSnapshotJson` (full `ScoringRunSnapshot` parts 1-8 and resolved max HR), and `scoringSnapshotId` (SHA-256 of snapshot JSON). Stored in `resync_checkpoint.proto` field 12. |
...
| `app/src/main/proto/resync_checkpoint.proto`                                            | Ingestion — checkpoint schema                       | Resync phase/date; proto field 12 run_identity for protocol v2 immutable run snapshots  |
| `core/model/src/main/kotlin/app/readylytics/health/core/model/domain/sync/HistoricalRunIdentity.kt`                      | Ingestion — immutable run model                     | Protocol v2 immutable run identity binding mode, dates, zone, and snapshot IDs          |
| `core/model/src/main/kotlin/app/readylytics/health/core/model/domain/sync/ScoringRunSnapshot.kt`                        | Ingestion — scoring preferences snapshot            | Canonical serialization parts 1-8 of preferences and source selection for run identity  |

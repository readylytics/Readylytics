...
└──────────────┬───────────────┘
               ...
               ▼
┌──────────────────────────────────────────────────────────────────────────┐
│  core/healthconnect/.../domain/sync/HealthSyncUseCase — thin facade;       │
│  acquires the application-scoped HealthMutationCoordinator                 │
...
│   • HealthMutationCoordinator.withMutation — serializes every health-data  │
│                               mutation against maintenance and each other   │
...
| `HealthConnectRepository` (interface)        | `core/model/src/main/kotlin/app/readylytics/health/core/model/domain/repository/HealthConnectRepository.kt`        | Extends `HealthConnectPermissionChecker`. Declares permission sets (critical / required / optional), `checkPermissions() → PermissionStatus` (Granted / Unavailable / Missing), and per-type read methods. Returns app-owned DTOs from `core/model/src/main/kotlin/app/readylytics/health/core/model/domain/model/HealthConnectRecords.kt`, not Android Health Connect SDK record types, wrapped in `ReadOutcome<T>` (`Available(data)`, `Denied`, `Unsupported`). This strictly distinguishes denied or unsupported reads from genuine empty data without conflation or premature deletion. Feature status for historical and background data is probed explicitly (`isHistoryReadAvailable()`, `isBackgroundReadAvailable()`). Deletion authority is granted strictly to `ReadOutcome.Available` reads; `CancellationException` is preserved and never swallowed. Denied change-sync types are suspended in `HealthChangeTokenStore` (`suspendType`, `isSuspended`) to avoid full-resync loops while preserving existing data. |
...
| `StepRecordReader`                    | `core/healthconnect/src/main/kotlin/app/readylytics/health/core/healthconnect/data/healthconnect/StepRecordReader.kt`            | Collaborator for reading raw step records, single-range aggregated step counts (`readSteps`), and calendar-day grouped step totals (`readDailyStepTotals`, HC-003) via `aggregateGroupByPeriod` with fallback to per-day aggregation. Receives the singleton `HealthConnectClient` from `HealthConnectModule`; it never calls `getOrCreate` itself. |
| `IntervalTotalsReader`                | `core/healthconnect/src/main/kotlin/app/readylytics/health/core/healthconnect/data/healthconnect/IntervalTotalsReader.kt`        | Collaborator for bulk-reading interval totals (distance and elevation gain) and resolving cumulative metrics for exercise sessions via `SessionTotalsResolver`. Receives the same injected client as the repository. |

`HealthConnectModule` owns the only production `HealthConnectClient.getOrCreate` call. Hilt supplies
that singleton client to `HealthConnectRepositoryImpl`, `StepRecordReader`, and
`IntervalTotalsReader`; readers do not construct a client locally. Runtime availability and permission
probes still return `ReadOutcome.Unsupported`/`Denied` without making startup depend on Health Connect.
...
**HC-005/PERF-001 — one shared retry budget per ingest window:** `HealthIngestionCoordinator.ingestWindow`
creates a single `ReadRetryBudget` (`domain/sync/ReadRetryBudget.kt`) fresh for each window and
threads it through `IngestWindowParams`. Every read inside that window -- the 9 bulk
`fetchBulkRecords` reads and `HeartSampleStreamer`'s HR/HRV paged reads -- calls
`retryBudget.execute(label) { … }` and draws down the *same* shared attempt count (`maxAttempts`,
`HealthConnectRetryPolicy`'s default 5) instead of each read getting its own independent budget.
Once the window's budget is spent, the failure that spent it is replayed immediately for any
further read in that window, without invoking it. This replaces the old shape where
`HistoricalIngestPhase` additionally wrapped the *entire* `ingestWindow` call in its own
`retryWithBackoff`, nesting outer (window) and inner (per-read) retries so a provider under quota
pressure could be hit up to `maxAttempts^2` times for one window; `HistoricalIngestPhase` no longer
wraps `ingestWindow` at all -- see 1.2's `HistoricalIngestPhase` row. `retryWithBackoff` itself
(`RetryWithBackoff.kt`) gained an optional `budget: ReadRetryBudget? = null` parameter that, when
supplied, delegates entirely to the budget; callers that don't supply one (e.g. `StepCountFetcher`,
and the Changes-token page-fetch path below) keep their prior independent-attempt-count behavior
unchanged.

**Changes-token path keeps its own, separate budget:** `HealthChangeSynchronizerImpl`'s
`client.getChanges(currentToken)` calls (recent-window and per-interval-type change sync) are a
different read with a different failure meaning than a bulk/paged window read, and are not part of
the window-scoped `ReadRetryBudget` described above -- they are untouched by this change.

**Paginated Fetch, Streaming & Token Resumption:**
High-volume vitals (Heart Rate and HRV) stream page-by-page via `readHeartRateSamplesPaged` and
...
| Component                     | Path                                         | Responsibility                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| :---------------------------- | :------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `HealthSyncUseCase`           | `core/healthconnect/src/main/kotlin/app/readylytics/health/core/healthconnect/domain/sync/HealthSyncUseCase.kt`           | Facade using `HealthMutationCoordinator.withMutation`; its public methods delegate to `DailySyncUseCase` (`sync`), `ResyncRangeUseCase` (`resyncRange`, `recomputeRange`), and chunked catch-up resync (`catchUpSync`), which carry the behavior described in this row. `catchUpSync` is gated by `lastSyncTimestamp == 0` and executes `resyncRangeUseCase.run` in 30-day chunks over a 365-day range. `recomputeRange(start, end, onProgress)` (SCORE-007, see 1.2.2) runs under the shared `HealthMutationCoordinator` and delegates to `resyncRangeUseCase.run(..., skipIngestAndPrune = true)` — a Health-Connect-free re-scoring pass for settings changes that alter historical scoring inputs (TRIMP model/params, HR zones, hrMax, RAS scaling factor) without altering ingested data. Its resumable identity includes a versioned canonical scoring-preference fingerprint, so a changed scoring snapshot invalidates the saved checkpoint and restarts at the requested range start; recompute-only checkpoints contain no Changes API tokens. Startup baseline backfill uses the same coordinator boundary, so it never races a sync/resync. `sync(windowDays, onProgress)` recent-window sync — note the **ingestion fetch starts one day earlier than the scored window** (`today − windowDays`), because overnight sleep sessions begin the previous evening; clipping at the scored window's midnight would drop a night's pre-midnight HR/HRV samples (lower HRV mean, higher RHR percentile). After recent-window ingest, `sync()` runs `SessionLinkReconciler.reconcile(...)` over the ingested overlap before scoring so pull-to-refresh preserves the same canonical HR/HRV session links as historical resync. Raw Health Connect reads stay in the sync engine, but Room writes now flow through `HealthIngestionStore.persist(batch)` and frozen-baseline clearing through `HealthIngestionStore.clearFrozenBaselines(start, endExclusive)`, keeping DAO/transaction details in data layer. The recalc loop covers `windowDays`, widened down to the earliest **recent** out-of-window affected day (within `MAX_INLINE_RECOMPUTE_DAYS` = 7 of today, in `DailySyncUseCase`) so last night's sleep (dated yesterday) or backfilled HR/HRV recomputes inline; only changes older than that inline bound escalate to durable historical resync. `resyncRange(start, end, chunkDays = 30, onProgress)` full historical runs **four resumable phases**: chunked ingest + deletion reconciliation → selected-source prune → full-range session-link reconcile → walk-forward recompute. Before first ingest it captures Changes API baseline tokens and stores them with the immutable range, current phase, next date, and device-selection hash in `ResyncCheckpointStore`; retries reuse the same baseline, while legacy/mismatched checkpoints restart cleanly. **Every ingestion chunk of `resyncRange` starts one day early and fetches sleep/exercise sessions one day past the chunk end** so HR/HRV samples at either side of a 30-day boundary can still be assigned to cross-midnight sessions. Metric sample reads remain capped to the chunk. Ingestion phase also executes chunk-scoped deletion reconciliation (R2-HC-001, WP-18): records in Room within the chunk window absent from staged Health Connect scan IDs are deleted in transaction via anti-join queries against `scan_seen_ids`, converging on Health Connect deletions without blanket `deleteAll()`. A scan that is not `COMPLETE` prunes nothing (HC-002). Step totals are rebuilt only for the remaining recompute range on resume, preserving selected-device correctness without re-running completed raw ingest. After all chunks are ingested, a single `SessionLinkReconciler.reconcile(...)` pass starting one day before the start date (i.e. `start - 1 day` to `end`) re-derives HR/HRV session linkage and recomputes affected workout metrics (see 1.2.1) — this makes the result independent of chunk alignment. Historical resync progress reports recomputed calendar days, not internal ingest/prune/reconcile units, but resumed recompute starts with the already-completed day offset. `HealthIngestionCoordinator.ingestWindow(start, end, prefs)` remains the single read→map→filter funnel (shared by both flows); `StepCountFetcher` performs per-device step reads (recent window + historical range); `DailyRecomputeSupport` runs the per-day score recompute and auto-MaxHR refresh; `retryWithBackoff` (shared helper in `RetryWithBackoff.kt`, `HealthConnectRetryPolicy`'s default `maxAttempts = 5`) handles transient HC/IO faults for independent-scope callers (never swallows `CancellationException`) and, for one ingest window's reads, delegates to that window's shared `ReadRetryBudget` (HC-005/PERF-001, see 1.1) instead of retrying independently per read; The coordinator serializes daily sync and resync. |
| `DailySyncUseCase`            | `core/healthconnect/src/main/kotlin/app/readylytics/health/core/healthconnect/domain/sync/DailySyncUseCase.kt`            | Foreground daily-sync orchestrator (the `sync(windowDays)` body). Acquires the shared `HealthMutationCoordinator` through the facade. Owns the recent-window ingest → reconcile-over-overlap → frozen-baseline clear → walk-forward recompute, the cross-midnight reach-back, change-token commit, `lastSyncTimestamp` update on completed success (R2-HC-005), and the `REQUIRES_HISTORICAL_RESYNC` decision. Before the walk-forward it builds one `WalkForwardTrimpContext` + `WalkForwardBaselineContext` + `WalkForwardVo2MaxContext` over the widened `[oldestTargetDay, today]` range (`DailyRecomputeSupport.buildWalkForward*`) and passes them to every recomputed day, so the 84-day TRIMP series, 56-day baseline sleep window, and 30-day wearable VO2 Max lookback are fetched once per sync rather than once per synced day — the same PERF-002/WP-20/WP-22 shape `ResyncRangeUseCase` already uses. The frozen-baseline clear runs in an initial `DailyRecomputeSupport.inRecomputeTransaction { }`, followed by per-day recompute transactions (R2-CACHE-002), so cancellation or interruption commits already-calculated days cooperatively without rolling back the entire multi-day window. Health Connect I/O (window ingest, reconcile, step fetch) always completes before those transactions open. The recent-window ingest is split into today's segment `[todayMidnight, windowEnd)` and the overnight back-day reach-back `[ingestStart, todayMidnight)` (B′), each under its own `withTimeout` budget; a segment that times out is retried once with an extended budget, a still-failing today segment returns `DEFERRED_DAILY_SYNC` (no historical-worker escalation), and a still-failing back-day segment is best-effort so today still scores. Cancellation leaves already completed days committed; the next sync redoes the remaining idempotent range. **Back-day ticket absorption:** after ingest (which can journal a dirty ticket whose cursor is the reach-back day, because a changed HR/HRV source that starts before midnight dates its ticket there) and before the walk-forward, `DailyRecomputeSupport.resolveRecomputeStartDay(oldestTargetDay, inlineFloor)` lowers the walk-forward start to the earliest pending ticket cursor in `[today − MAX_INLINE_RECOMPUTE_DAYS, oldestTargetDay)`. `DirtySummaryPublisher` only advances a ticket when its cursor day is published, so without this the ticket never drained and every later process start enqueued a background recompute (`STARTUP_PENDING_DIRTY`). Tickets older than the inline floor stay with the durable worker. A walk-forward longer than `LARGE_INLINE_WINDOW_DAYS` (3) is logged at INFO with the dates that widened it. `REQUIRES_HISTORICAL_RESYNC` failures carry the cause in `Result.Failure.reason` (`HealthChangeSyncOutcome.fullResyncReason` — missing/expired change token — or the out-of-floor affected dates), which the escalating caller forwards as the resync's `triggerDetail`. |
| `ResyncRangeUseCase`          | `core/healthconnect/src/main/kotlin/app/readylytics/health/core/healthconnect/domain/sync/ResyncRangeUseCase.kt`          | Full-historical-resync orchestrator (the `resyncRange(...)` body). Acquires the shared `HealthMutationCoordinator` through the facade. Owns the four resumable phases (chunked ingest + deletion reconciliation → selected-source prune → full-range session-link reconcile → walk-forward recompute), immutable run identity resolution (`HistoricalRunResolver`), checkpoint capture/resume, and baseline-token promotion. Delegates chunked ingestion to `HistoricalIngestPhase` and walk-forward recompute to `HistoricalRecomputePhase`. Operates under protocol version 3 (`HistoricalRunIdentity`): captures immutable run identity with SHA-256 snapshots of preferences and source selections before work begins. On resume, if the saved run identity matches requested inputs (mode, zoneId, algorithmRevision, sourceSelectionId, scoringSnapshotId, startEpochDay) and the requested end date is greater than or equal to the saved run's end date (e.g. crossing midnight or daylight saving time in Europe/Berlin), the saved run is preserved; otherwise the checkpoint restarts cleanly from the requested start date. Stored page tokens (`checkpoint.hrPageToken`/`checkpoint.hrvPageToken`) are forwarded into `ingestWindow` on resume, each independently setting `resumeHrScan = (hrStartPageToken != null)` / `resumeHrvScan = (hrvStartPageToken != null)` (HC-002 remediation) — a chunk whose HR token is present but whose HRV scan already completed (or vice versa) resumes only the type that actually has a pending token, rather than forcing both types to replay together; because `ScanStagingStore` (`scan_seen_ids` in Room / `InMemoryScanStagingStore` in tests) stages scanned record identities per page directly into persistent staging, mid-chunk page token resumption preserves deletion authority across earlier streamed pages of the chunk without holding accumulators on the heap. If the Health Connect provider rejects a stored page token (HC-005), the ingestion phase catches the failure and safely falls back to replaying the entire chunk from scratch (`startPageToken = null`, `resumeHrScan = false`, `resumeHrvScan = false`). Tracks `completedTypes` across all chunks, gated by `completedTypesRecorded`. `run(..., skipIngestAndPrune = false)`: when `true` (see 1.2.2), ingest, prune, and deletion reconciliation are forced off, the checkpoint starts at `RECONCILE`, and the run identity uses `MODE_RECOMPUTE_ONLY`. Matching scoring snapshots resume; changed scoring inputs restart from the requested start date, while operational/UI settings do not invalidate the checkpoint. Recompute-only checkpoints deliberately store no Changes API tokens and never capture/apply/commit tokens or update `lastSyncTimestamp`; full-resync checkpoints retain the mandatory non-empty-token resume guard. |
| `HistoricalIngestPhase`       | `core/healthconnect/src/main/kotlin/app/readylytics/health/core/healthconnect/domain/sync/HistoricalIngestPhase.kt`       | Chunked ingestion loop extracted from `ResyncRangeUseCase` to enforce line-count bounds (WP-10). Executes chunked Health Connect reads, adaptive chunk shrink on `HealthConnectWindowTimeoutException` (halving effective chunk size down to 1 day, persisted as `chunkDaysOverride`), in-transaction deletion reconciliation via `HealthIngestionStore.reconcileWindow`, and intermediate `INGEST` checkpoints carrying the immutable `runIdentity`. **HC-005/PERF-001:** `runIngestWindow` no longer wraps its `ingestWindow` call in `retryWithBackoff` -- that window-level retry is gone now that every read inside `ingestWindow` shares one bounded `ReadRetryBudget` (see 1.1), so a budget exhaustion propagates straight out of `processChunk` to `ResyncRangeUseCase`/`HealthResyncWorker`, which returns `Result.retry()` and relies on WorkManager's own `EXPONENTIAL` backoff (`WorkerSchedulerImpl`) as the sole remaining outer retry. The chunk-shrink-on-timeout branch above and Task 4's rejected-page-token replay (`runWithTokenFallback`) remain the only in-process recovery in this phase. |
| `HistoricalPrunePhase`        | `core/healthconnect/src/main/kotlin/app/readylytics/health/core/healthconnect/domain/sync/HistoricalPrunePhase.kt`        | Selected-source prune phase extracted from `ResyncRangeUseCase` (WP-10), the second of the four resumable phases. Builds the current per-type device selection map (`HealthDataType.entries` associated with `prefs.deviceByDataType`) and calls `SelectedSourcePruner.prune(start, endInclusive, selections, zoneId)` to delete any already-ingested record whose device no longer matches the selected source for its type — this recovers from a source-selection change even though `HealthIngestionCoordinator` already filters ingestion to the selected device, since data ingested under a prior selection persists until pruned. Not gated by `completedTypes`: it always prunes the full requested range for every type using the *current* preferences, regardless of which types finished ingestion this run. On completion, saves the checkpoint at phase `RECONCILE` with `nextDate` reset to the run's start date, carrying `runCompletedTypes` and the immutable `runIdentity` forward unchanged. |
| `HistoricalRecomputePhase`    | `core/healthconnect/src/main/kotlin/app/readylytics/health/core/healthconnect/domain/sync/HistoricalRecomputePhase.kt`    | Transactional walk-forward recompute loop extracted from `ResyncRangeUseCase` (WP-10, WP-21). Executes daily recompute across the historical range with publication units bounded to 1-day transactions via `DailyRecomputeSupport.inRecomputeTransaction { }`, checkpointed at 30-day intervals (`RECOMPUTE_CHECKPOINT_INTERVAL_DAYS`). Each day's daily summary, staged workout updates, and dirty tickets are atomically committed in `DirtySummaryPublisher.publishDay`, while checkpoints advance only after committed units. Retains the immutable `runIdentity` across checkpoints. |
| `HistoricalRunIdentity`       | `core/model/src/main/kotlin/app/readylytics/health/core/model/domain/sync/HistoricalRunIdentity.kt`                        | Immutable domain model (protocolVersion = 3) binding a historical run to its input environment: `runId`, `mode` (`FULL_INGEST` / `RECOMPUTE_ONLY`), `startEpochDay`, `endEpochDayInclusive`, `zoneId`, `startedAtEpochMs`, `sourceSelectionId` (SHA-256 of sorted device selections), `algorithmRevision`, `scoringSnapshotJson` (full `ScoringRunSnapshot` parts 1-9 and resolved max HR, including retention settings), and `scoringSnapshotId` (SHA-256 of snapshot JSON). Stored in `resync_checkpoint.proto` field 12; protocol-2 checkpoints restart safely. |
| `HistoricalRunResolver`       | `core/healthconnect/src/main/kotlin/app/readylytics/health/core/healthconnect/domain/sync/HistoricalRunResolver.kt`       | Determines whether an existing persisted resync checkpoint can be safely resumed or must restart cleanly. Preserves existing run if protocol version is 2 and all identity parameters (mode, zoneId, algorithmRevision, sourceSelectionId, scoringSnapshotId, startEpochDay) match, and `requested.endEpochDayInclusive >= existing.endEpochDayInclusive` (allowing runs that span across midnight or DST shifts to safely resume). Returns the new requested identity on any mismatch or older protocol. |
| `HealthIngestionCoordinator`  | `core/healthconnect/src/main/kotlin/app/readylytics/health/core/healthconnect/domain/sync/HealthIngestionCoordinator.kt`  | Single read→map→device-filter→upsert funnel for one HC window (`ingestWindow(windowStart, windowEnd, prefs, scanIdentity, resumeHrScan, resumeHrvScan, windowBudgetMs, onProgress, hrStartPageToken, hrvStartPageToken, onTokenUpdated, reconcileDeletions)`), shared by both flows. `onProgress` (optional, default `null`) fires `(ResyncPhase.INGEST, pagesIngested, 0)` after each HR/HRV page persists — an indeterminate running page count, not a determinate total; `DailySyncUseCase` passes its own `onProgress` through here, `ResyncRangeUseCase` reports chunk-level `INGEST` progress separately and does not thread per-page granularity through this parameter. Sessions and low-volume record types (the 9 bulk `fetchBulkRecords` reads — sleep, exercise, weight, body fat, blood pressure, SpO2, body temperature, steps, VO2 Max — each independent of the others' results, so they run concurrently via `coroutineScope`/`async` rather than sequentially, and each retried through this window's single shared `ReadRetryBudget`, HC-005/PERF-001, see 1.1) are fetched and persisted first, in one `HealthIngestionStore.persist(batch)` transaction, and their scanned record IDs are staged to `ScanStagingStore` (`scan_seen_ids` in Room / `InMemoryScanStagingStore` in tests). The vitals half of that map+device-filter step (weight, body fat, blood pressure, SpO2, body temperature) lives in `VitalsInputMapper` (`core/healthconnect/.../domain/sync/VitalsInputMapper.kt`) — pure mapping with no coordinator state, extracted so `HealthIngestionCoordinator.kt` stays inside the "extract rather than grow it" size constraint; row ids remain `"${hcId}_${timestampMs}"` with the opaque HC record id carried separately as `sourceId`, so upsert idempotency is unchanged. HR and HRV sample streaming, staging, and per-page persistence is delegated to `HeartSampleStreamer` (HC-001, R2-HC-002), which streams via `readHeartRateSamplesPaged`/`readHrvSamplesPaged`, maps each page to complete-source containers (`SourcePayload<HeartRateInput>`/`SourcePayload<HrvInput>`) tagged against this window's already-known sessions, stages scanned record identities directly to `ScanStagingStore` per page, and persists immediately through `HealthIngestionStore.replaceHeartRateSources`/`replaceHrvSources` (delegating to `SourcePayloadWriter` to authoritatively replace complete source payloads and sub-batch at ≤500 rows per transaction). This eliminates heap `Set<String>` accumulators and ensures at most one HC page of samples is held in memory at once. Individual raw HC page fetches are retried via `retryWithBackoff` inside `HealthConnectRepositoryImpl.readAllPagesStreaming` (unchanged by HC-005/PERF-001), while each *whole* `readHeartRateSamplesPaged`/`readHrvSamplesPaged` call (labels `hrPages`/`hrvPages`) is retried through this window's shared `ReadRetryBudget`, and pages notify `onTokenUpdated` with the next page token so mid-chunk streamed reads can resume -- HR and HRV independently, each type's own `resumeHrScan`/`resumeHrvScan` flag set only when that type's own stored page token is present -- without losing deletion authority across earlier streamed pages of the chunk (R2-HC-002). When `reconcileDeletions` is true (R2-HC-001, WP-06, WP-18), it performs deletion reconciliation against Room via `HealthIngestionStore.reconcileWindow`, taking strongly typed `CompleteTypeScan(type, windowStartMs, windowEndExclusiveMs, sourceSelectionId, scan: ScanIdentity)`. Deletion reconciliation delegates to `StagedDeletionReconciler` using Room set-based anti-join queries against `scan_seen_ids` (`boundsOfUnstaged*` to compute aggregate affected bounds, followed by `delete*NotStaged` without bind-variable lists, plus keyset-paged `pageUnstagedAuthoritativeSources` for HR/HRV parents). Under HC-002, reconciliation explicitly verifies `scan_type_state` and returns `null` without pruning or mutating any local records unless the scan state for that type is `COMPLETE`. If the Health Connect provider rejects a stored page token on resumption (HC-005), the ingestion phase catches the rejection and replays the chunk from scratch with clean staging (`startPageToken = null`, `resumeHrScan = false`, `resumeHrvScan = false`). Returns `IngestionWindowResult(affectedRange, completedTypes)`. Workouts are persisted with zero HR-derived metrics at this point (mirroring the changes-path pattern); `SessionLinkReconciler.recomputeWorkouts`, which both sync flows always run immediately after ingestion, fills in the real values once every HR sample in range is stored. The entire window read (sessions, low-volume types, and both streamed passes) runs inside one `withTimeout(windowBudgetMs)`; a `TimeoutCancellationException` from that timeout is caught here and rethrown as `HealthConnectWindowTimeoutException` (HC-002) — deliberately not a `CancellationException` subtype, so callers can never mistake "this window is too dense for its budget" for cooperative cancellation. If interrupted, the checkpoint stays on the current HC window (with active HR/HRV page tokens) and stable-ID upserts make the retry safe without deleting prior valid data. **H3/WP-08:** VO2 Max is device-filtered before persist (`DeviceSourceFilter.filterToDevice` against `deviceByDataType[VO2_MAX]`) exactly like weight/body fat/etc, and contributes its own `CompleteTypeScan(HealthDataType.VO2_MAX, …)` built from the *unfiltered* raw read (so a record still present in HC for a non-selected device is never wrongly reconciled away, matching every other device-selectable type). |
| `StepCountFetcher`            | `core/healthconnect/src/main/kotlin/app/readylytics/health/core/healthconnect/domain/sync/StepCountFetcher.kt`            | Per-device daily step reads. `fetchWindow(...)` for the recent window (semaphore-capped concurrent reads when no device filter); `fetchRange(...)` for the resync recompute range — the "all devices" path issues one grouped `readDailyStepTotals` call per chunk (HC-003) instead of one `readSteps` aggregate call per calendar day; the device-selected path stays raw-record-based, chunked, retry-wrapped, deduplicating IDs across chunk boundaries and clipping published totals to requested days. Cross-midnight intervals retain endpoints in `StepsMapper.StepEntry` and attribute counts to their start day under OD-6 policy. |
...
| `MainActivity` / `HealthDashboardApplication` startup gate | `app/src/main/kotlin/app/readylytics/health/MainActivity.kt`; `app/src/main/kotlin/app/readylytics/health/HealthDashboardApplication.kt`; `app/src/main/kotlin/app/readylytics/health/DatabaseReadyStartupInitializer.kt` | Both entry points observe the domain-facing `DatabaseMigrationController` before resolving Room-backed graphs. `MainActivity` renders a blocking Material 3 migration screen for every non-ready state through the dependency-free `DatabaseReadinessTheme`, starts/resumes required migration once from `LaunchedEffect`, and creates the preference-backed `ThemeViewModel`, `SyncViewModel`, or resolves the lazy `LocalRestoreManager` only in the `Ready` branch. The application injects `BackfillHistoricalBaselinesUseCase`, and the broad `SettingsRepository` as `dagger.Lazy`; settings are indirectly Room-backed through `UIPreferences` → `HealthDeviceRepository` → DAOs, so the initializer resolves all three lazies only after `DatabaseReadiness.Ready`. An `AtomicBoolean`-guarded initializer then runs baseline backfill under `HealthMutationCoordinator.withMutation` and schedules backup, birthday, and cleanup work exactly once per completed process initialization. After the TRIMP defaults preference migration, the initializer independently checks the recompute gates even if that migration throws; its failure is logged but cannot suppress healing. It enqueues at most one durable background recompute (`WorkerScheduler.scheduleResyncWorker(recomputeOnly = true)`) when `storedScoringVersion < CURRENT_SCORING_VERSION`, retained workout data still has unbackfilled canonical TRIMP (`WorkoutTrimpBackfillStatus`), or pending dirty tickets remain, in that trigger priority order. The worker owns convergence and the version bump. Scoring version 4 uses this ordinary retained walk-forward—not the parameter-only projection branch—so existing v17 rows rebuild Residual Fatigue chronologically and receive default projections. Only successful worker completion records version 4/applied defaults. The backfill gate uses the same stored-scoring-zone instant as cleanup and the worker's recompute start, never device/system midnight. Periodic sync is scheduled at the stored interval only when `backgroundSyncEnabled`; otherwise its unique work is cancelled. An ordinary incomplete initialization resets the guard and returns a retryable status; the application coordinator retries at bounded 500 ms / 2 s / 8 s delays while the current readiness remains Ready, without depending on another equal `StateFlow` emission. `collectLatest` cancels that retry chain as soon as readiness changes; cancellation resets the guard and is always rethrown. A v5/v6 database therefore cannot be opened indirectly by normal startup while the external migration owns the file. |
| `WorkerScheduler`             | `core/model/src/main/kotlin/app/readylytics/health/core/model/workers/WorkerScheduler.kt`                 | Enqueues unique work. `scheduleDatabaseMigration()` owns the distinct v7 migration chain described above. `scheduleResyncWorker(recomputeOnly = false, startDate = null, endDate = null)` uses the shared `RESYNC_WORK_NAME` chain, expedited execution, and exponential backoff; explicit full resync uses `ExistingWorkPolicy.KEEP`, while recompute-only settings requests use `ExistingWorkPolicy.APPEND_OR_REPLACE` to append a durable successor. The request type is passed through as `KEY_RECOMPUTE_ONLY` input data (see 1.2.2). **R2-CACHE-001:** `startDate`/`endDate`, when both non-null, carry a bounded recompute-only date range (as epoch days, `KEY_RECOMPUTE_START_EPOCH_DAY`/`KEY_RECOMPUTE_END_EPOCH_DAY` input data) through to `HealthResyncWorker`/`FullHistoricalResyncUseCase`; left `null` (every pre-existing call site), the recompute-only pass keeps covering the full retention window as before. No production caller passes a range any more (aging data out no longer invalidates retained summaries, see `DataCleanupWorker`/`DataRollupWorker`); the parameters remain for bounded recompute requests. `trigger: RecalcTrigger` (default `SETTINGS_CHANGE`) and optional `triggerDetail` are written as `KEY_TRIGGER`/`KEY_TRIGGER_DETAIL` input data (detail capped at 1,000 chars) for `RecalcDiagnosticRecorder`; callers: startup gate (`STARTUP_SCORING_VERSION`/`STARTUP_TRIMP_BACKFILL`/`STARTUP_PENDING_DIRTY`), `PeriodicHealthSyncWorker` (`PERIODIC_SYNC_ESCALATION`), `ForegroundSyncController` (`FOREGROUND_SYNC_ESCALATION`, `CATCH_UP_CAP`), Settings resync (`USER_RESYNC`). Multiple rapid settings changes may create redundant local passes, but the newest preferences are eventually captured without adding a second work name or progress channel. `scheduleTrainingReadinessRecompute(config)` captures immutable S/w in a `TRAINING_READINESS` request on the same unique chain with `APPEND_OR_REPLACE`; it adds no new work name or progress channel. Also provides `cancelResyncWorker()`; `schedulePeriodicSync(intervalMinutes)` (`PERIODIC_SYNC_WORK_NAME`, `ExistingPeriodicWorkPolicy.UPDATE`, exponential backoff, requires battery not low but no charging or device-idle constraint) + `cancelPeriodicSync()`; and backup / birthday / data-cleanup workers. |
| `PeriodicHealthSyncWorker`    | `app/src/main/kotlin/app/readylytics/health/workers/PeriodicHealthSyncWorker.kt`        | "Background Sync" toggle in Settings. Before resolving its lazy Room-backed `HealthSyncUseCase` or lazy `ForegroundSyncController`, it requires `DatabaseReadiness.Ready`; otherwise it retries without opening Room or constructing the controller graph. Once ready, this `@HiltWorker` periodic **standard (non-foreground) worker** calls `HealthSyncUseCase.sync(windowDays = 2)` (shares `HealthMutationCoordinator` with the other two flows), bridges progress to `ForegroundSyncController`, shows/dismisses a silent transient notification (`SyncNotifications.BACKGROUND_SYNC_CHANNEL_ID`) via `NotificationManagerCompat` directly — no `setForeground()`, since `READ_HEALTH_DATA_IN_BACKGROUND` already permits background HC reads and starting a foreground service from a periodic background worker risks `ForegroundServiceStartNotAllowedException` on API 34+. Ordinary failures return `Result.retry()`; `REQUIRES_HISTORICAL_RESYNC` enqueues `WorkerScheduler.scheduleResyncWorker()` and finishes successfully so durable catch-up runs once without periodic retry churn. |
| `LocalBackupWorker`           | `app/src/main/kotlin/app/readylytics/health/workers/LocalBackupWorker.kt`                | Before resolving its lazy Room-backed `LocalBackupManager`, it requires `DatabaseReadiness.Ready`; otherwise it retries without opening Room. Ready runs preserve the existing local encrypted backup result mapping. Backups stage directly into a private encrypted ZIP (`cacheDir/backup-staging/`) under an operation UUID via `BackupSnapshotExporter` while holding the P2 maintenance gate (`HealthMutationCoordinator.withMaintenance`) to freeze a coherent logical generation without intermediate plaintext JSON. `BackupStreamWriter` captures pre-admitted preferences, layouts, and `BackupSnapshotIdentity` and streams `health_source_records` via keyset pagination (`pageAfter`). Publication read-back and inventory validation using `RestoreInventoryValidator` are completed before SAF export and pruning existing archives from the active store, retaining prior recovery points on failure. Orphan staging files are cleared on startup by `CachePrune` / `DatabaseReadyStartupInitializer`. |
| `DataCleanupWorker`           | `app/src/main/kotlin/app/readylytics/health/workers/DataCleanupWorker.kt`               | Daily retention enforcement; before resolving lazy `RetentionCleanup`, it requires `DatabaseReadiness.Ready` and retries without opening Room otherwise. Cutoff is `RetentionBounds.resolveHistoricalWindow(prefs).startTimeMs`, exposed via `resolveRetentionCutoffMs`: scoring-zone midnight shared with resync/startup, never device-zone midnight. No-op when retention is disabled. **Aging never invalidates retained summaries:** a non-empty deletion is only logged; it enqueues no recompute (and `RetentionCleanup` journals no dirty work). Each retained day was scored and its baselines frozen while the older data still existed, so recomputing it would only drop inputs — the former R2-CACHE-001 fan-out re-ran an ~84-day recompute every night the cutoff advanced. |
| `DataRollupWorker`            | `app/src/main/kotlin/app/readylytics/health/workers/DataRollupWorker.kt`                | Daily hot→warm rollup; resolves the 90-day `RetentionBounds.resolveHotTierCutoffMs()` and delegates to `DataRollupManager`. `Result.retry()` on transient failure. **Aging never invalidates retained summaries:** a non-empty rollup is only logged; it enqueues no recompute (and `DataRollupManager` journals no dirty work). Retained days were already scored from the full-resolution raw samples; re-deriving them from the warm-tier approximation would only lose accuracy, and the former R2-CACHE-001 fan-out re-ran an ~84-day recompute every night the 90-day boundary advanced. |
| `DataRollupManager`           | `core/database/src/main/kotlin/app/readylytics/health/core/database/data/local/DataRollupManager.kt`  | Downsamples plausibility-filtered raw `heart_rate_records` older than the cutoff into `hr_minute_buckets` (min/max/avg/count plus a p5/p25/p50/p75/p95 percentile sketch, computed in Kotlin via `MinuteBucketAggregator` since SQLite has no `PERCENTILE_CONT`), then delegates the visibility switch to `MinuteCoveragePublisher` and deletes the consumed raw rows, day-chunked (R2-DB-004, OD-1). **PERF-003 (Task 8):** within a day chunk, `MinuteRollupStreamer` streams `HeartRateDao.pagePlausibleSamplesForRollup` as keyset pages (`SAMPLE_PAGE_SIZE` = 5,000 rows) *outside any transaction*, holding back each page's trailing (possibly incomplete) minute until a later sample proves it closed, and hands the manager bounded groups of at most `GROUP_MINUTE_BUDGET` (60) *complete* minutes at a time — a group boundary can therefore never split a minute (the DB-002 defect class). Percentile/histogram aggregation (`aggregateIntoMinuteBuckets`) runs in Kotlin on that group, outside any writer transaction; only the actual publish + delete for that one group run inside a short transaction. `nextGeneration()` is captured once per GROUP, not once per day: `MinuteCoveragePublisher.publish` re-validates it inside that group's own transaction, so a concurrent source mutation (`SourceGenerationConflictException`) aborts only the group being published, never groups already committed earlier in the pass. The manager catches that exception, logs, and stops the whole pass cleanly (not a crash) — the next scheduled rollup retries the remaining groups idempotently. Because `aggregateIntoMinuteBuckets` sorts each minute's own BPM values before computing avg/percentiles, bucket output depends only on which samples share a minute, never on how many pages/groups a run was split into, so chunked and heavily-paged runs produce byte-identical buckets to a single-pass run. A minute whose only raw rows are implausible never forms a group (filtered by the same predicate the paged query shares with the old single-pass query); a conditional, day-scoped sweep at the end of a day chunk (skipped once nothing remains, so a clean day chunk opens exactly one transaction per group) still removes those stragglers, matching the old single-pass rollup's unconditional whole-day delete. Per complete minute it writes one `minute_coverage` row (`WARM`/`SOURCE_BACKED` at the current `sourceGeneration`) plus one `hr_source_minute_contributions` row per contributing `sourceRecordRef`, carrying that source's `bpmHistogram` (integer BPM frequency table), first/last observed sample timestamps and device. Contributions are **superseded, not immutable**: `generation` is part of their primary key, so the publisher deletes every prior generation's contributions AND bucket slices for exactly the minutes it republishes, inside the same transaction as the insert -- a minute can never be visible with two generations or two qualities. Re-publishing a minute re-derives it from the raw rows still present; the superseded generation's histogram evidence is discarded (same semantics the pre-WP-17 `upsertBuckets` REPLACE already had), so recovering a full minute needs an authorized complete refresh, not a rollup. **OD-1 (recorded decision, see the Phase-0/1 index decision ledger):** legacy warm rows stay approximate/unknown and only a complete authorized interval refresh may replace them -- so a minute whose coverage is still `LEGACY_UNKNOWN` is *quarantined* by an ordinary rollup: its coverage and legacy buckets are left untouched, no contributions are written for it, and its raw rows are deliberately NOT deleted (`HeartRateDao.deleteConsumedSamplesInRange` excludes those minutes) so the evidence survives until an authorized refresh. Consequence: for such a pre-existing overlap the hot tier keeps rows older than the cutoff; readers must prefer the published warm coverage and never concatenate both — enforced by `AuthoritativeHeartRateReader`, the single owner of that predicate (see "One tier and one generation per minute" in §1). The day-chunk cursor therefore advances on `max(dayEnd, nextEarliest)` rather than relying on deletion for progress. Generation is only advanced when a group has publishable minutes. **WP-17:** aligns the cutoff to `completeMinuteCutoff(cutoffMs)` to ensure partial minutes are never downsampled or deleted. `rollupExpiredHotTier` returns the `ScoreInvalidation.AffectedRange?` it actually touched (min/max dates of every rolled-up raw sample, merged across day-chunks and groups), or `null` on a no-op run — informational (logging) only. It writes no `dirty_ranges` ticket (the retired `HOT_TIER_ROLLUP` reason); the per-group generation capture/validation is kept as the concurrent-writer guard. |
| `MinuteCoveragePublisher`     | `core/database/src/main/kotlin/app/readylytics/health/core/database/data/local/MinuteCoveragePublisher.kt` | WP-17 Step 3/4 visibility switch for one complete minute/coverage unit. **Runs inside the caller's transaction and never opens its own**, so assembling the unit, switching visibility and consuming the raw evidence commit or abort together (proven by `MinuteCoveragePublicationTest`'s throw-before-commit / close-and-reopen case). In order: re-checks the captured `sourceGeneration` (typed `SourceGenerationConflictException` on drift), drops requested minutes whose coverage is still `LEGACY_UNKNOWN` and reports them as quarantined, deletes the superseded contributions and bucket slices of exactly the minutes it republishes (chunked `IN` on the minute keys, bounded to 500 so SQLite's variable limit is never approached), installs the new contributions and bucket projections, switches `minute_coverage`, then appends the P2 dirty range. Production callers are `DataRollupManager` (hot→warm rollup) and `WarmTierRelinker` (WP-17 Step 4 full-range relink, which republishes a re-keyed projection at the minute's existing `visibleGeneration` because the underlying evidence is unchanged). A complete authorized refresh of legacy minutes remains the third intended caller. |
| `AuthoritativeHeartRateReader` | `core/database/src/main/kotlin/app/readylytics/health/core/database/data/local/AuthoritativeHeartRateReader.kt` | WP-17 Step 3 single owner of the hot-versus-warm tier-visibility predicate. Read-only, opens no transaction of its own. Returns `AuthoritativeHrRange(rawSamples, warmBuckets)` — two collections guaranteed to describe **disjoint minutes**, plus `mergedSamples()`, which is what a consumer wanting "every sample in this window" must call (`rawSamples` alone is empty by design for a warm-covered minute; the merge reconstructs each visible bucket into raw-shaped rows keeping that bucket's own `recordType`/`sessionId`/`deviceName`) — plus session-scoped and minute-projection helpers (`sleepProjectionForSessions`, `sleepSamplesForSession`, `warmSessionSamples`, `minuteBuckets`, `rawByTypeInRange`, `minBpmInRange`) and `legacyApproximateMinutes(start, end)` for reporting how much of a window only an authorized complete refresh can repair. Backed by the mirrored `HeartRateDao.getVisible*` / `MinuteBucketDao.getVisible*` queries. Consumers are `HeartRateRepositoryImpl`, `ScoringHistoryRepositoryImpl`, `ScoringHeartRateDataLoader` and `SessionLinkReconcilerImpl` — public callers keep going through the existing repository interfaces. |
| `WarmTierRelinker`            | `core/database/src/main/kotlin/app/readylytics/health/core/database/data/local/WarmTierRelinker.kt` | WP-17 Step 4 full-range warm relink, called once per reconcile by `SessionLinkReconcilerImpl`. Walks covered minutes in day-chunks (each chunk one transaction; cursor advances via `MinuteCoverageSelectionDao.nextCoveredMinuteAtOrAfter`, a rowid seek, so uncovered history is skipped rather than walked), re-derives each `SOURCE_BACKED` minute's `(recordType, sessionId, deviceName)` slices from `reconstructEvidence()` + unchanged `SessionLinker`, re-aggregates via `MinuteBucketAggregator`, and republishes only changed minutes through `MinuteCoveragePublisher`. A uniform-minute fast path (whole minute inside one session, stored keys already match, stored sample count still equals the remaining evidence count) skips reconstruction entirely — the count comparison is what makes a *partial* source deletion converge. Retires minutes with no remaining evidence (buckets + coverage + every generation's contributions dropped together, so no orphan rows survive in `hr_source_minute_contributions`). Leaves `LEGACY_UNKNOWN` minutes untouched, and treats an unreadable `bpmHistogram` *or* an empty derivation as unresolved (preserved as-is and counted, never published as a guess; typed `BpmHistogramFormatException` is logged, never fatal to the pass). Returns `WarmRelinkOutcome(inspected, republished, retired, unresolvedLegacy, unresolvedEvidence)`, which `SessionLinkReconcilerImpl` logs once per reconcile. Cooperative (`ensureActive()`/`yield()` per chunk). |
| `HeartRateRefreshStagingStore`| `core/database/src/main/kotlin/app/readylytics/health/core/database/data/local/HeartRateRefreshStagingStore.kt` | WP-17 Step 4 staging store for an in-flight authorized refresh. `stage(runId, sourceId, metadata, samples)` replaces that staged parent's rows in one transaction: the metadata row is written with `payloadComplete = false`, the children are inserted, then the metadata row is rewritten with the **caller's own** `payloadComplete` — so a crash mid-insert can never leave a partial payload marked complete, and a payload the caller knows to be partial stays incomplete. Staged rows live in `staged_hr_sources`/`staged_hr_samples`, keyed textually by `(runId, sourceId)` with no FK into `health_source_records` (staging must never mutate live authoritative metadata just to obtain one). An empty complete parent keeps its metadata row. Staged work is durable operational state and grants no prune or visibility authority by itself; it is excluded from exported health data, and dirty work is regenerated on restore. |
| `BpmHistogram`                | `core/database/src/main/kotlin/app/readylytics/health/core/database/data/local/BpmHistogram.kt` | Versioned (`v1:`) integer-frequency encoding of one source's plausible BPM samples for one minute, stored in `hr_source_minute_contributions.bpmHistogram`. Lossless frequency table, never a merge of percentile sketches: count/sum/average/min/max and percentiles all derive from the bins. `percentile(p)` reuses `PercentileUtils.weightedPercentileR7`, the weighted form of the R-type-7 interpolating `List<Int>.percentile` that `MinuteBucketAggregator` already writes into `p5Bpm..p95Bpm` — so reading a minute's percentiles back out of the histogram reproduces the stored warm-tier numbers instead of shifting a scoring input (equivalence asserted in `BpmHistogramTest`). `decode` raises the typed `BpmHistogramFormatException` for malformed payloads, out-of-domain BPM or negative counts. |
| `RetentionCleanup`            | `core/database/src/main/kotlin/app/readylytics/health/core/database/data/local/RetentionCleanup.kt`             | Executes deletions of data strictly older than the cutoff across all 13 sensitive tables. **DB-002:** `heart_rate_records` and `hrv_records` (the two high-volume tables) are deleted via `deleteBeforeTimestampBatch`, each call bounded to 10,000 rows and run in its own transaction, looping until a batch returns fewer than 10,000 deletes — so a large first-time cleanup opens many bounded transactions instead of one unbounded delete/WAL growth spike, and a killed worker mid-loop leaves already-deleted rows deleted (idempotent restart: `WHERE timestampMs < cutoff` simply matches fewer rows next time). The remaining 11 low-volume tables (`sleep_sessions`, `hr_minute_buckets`, `workout_records`, `daily_summaries`, `weight_records`, `body_fat_records`, `blood_pressure_records`, `oxygen_saturation_records`, `body_temperature_records`, `step_records`, `vo2_max_records`) are deleted together in one single transaction, as before. `deleteBefore` journals no dirty work and does not bump the source generation (the retired `RETENTION_CLEANUP` reason; it already runs under the `HealthMutationCoordinator` mutation). It returns — for logging only — the `ScoreInvalidation.AffectedRange?` it actually touched — the earliest pre-deletion timestamp across `heart_rate_records`/`hr_minute_buckets` (the two sources that feed the scoring walk-forward and change size with the rolling cutoff) through `cutoffMs`, read before deletion — or `null` when nothing was older than the cutoff. **PERF-003 (Task 9):** after every raw-row batch above has committed for the run — never before, so a source whose only children were deleted this same run is judged against the post-deletion state — calls `SourceMetadataGc.collect(daos.sourceRecordDao)` unconditionally (even on an otherwise no-op run, since orphans can also be left behind by other deletion paths, e.g. `SourceRecordDao.deleteBySourceRecordId`'s WP-17/OD-1 handling). `SourceMetadataGc` (`core/database/.../SourceMetadataGc.kt`) pages `SourceRecordDao.pageUnreferencedSourceIds` (keyset on `id`, `PAGE_SIZE` = 500, well under the 999 SQLite bound variable limit) and deletes each page via `deleteSourcesByRefs`, bounded to `LIMIT_PER_RUN` = 10,000 rows/run and cooperative (`ensureActive()`/`yield()` per page) like the batches above. A `health_source_records` row is deleted only when **both** halves of the safety rule hold. **(1) Eligible record type:** `+recordType IN ('HEART_RATE', 'HRV')` — an allowlist, not a denylist; the unary `+` hints the planner to prefer the `id > :afterRef` rowid keyset scan over `index_health_source_records_recordType_metadataState_recordStartMs`, since on an un-ANALYZE'd production DB SQLite otherwise picks that index for an un-hinted `recordType IN (...)` and loses the keyset page (see `Phase2QueryPlanTest.sourceGcCandidatePageDoesNotScanChildTables`). `health_source_records` stores two semantically different kinds of row: for HR/HRV the row is a *derived index* over raw children in `heart_rate_records`/`hrv_records`, but `RoomHealthChangeIngestionStore.persistIntervalEnrichment` also writes `DISTANCE`/`ELEVATION_GAINED` interval sources here (via `SourceRecordDao.upsertIntervalSourceRecord`, `metadataState = 'AUTHORITATIVE'`) where the row **is** the record — it carries the interval's own `recordStartMs`/`recordEndExclusiveMs`/`originPackage`/`lastModifiedMs` and has no children in any table, so every reference check in (2) would trivially pass for it. Collecting one would permanently erase primary data and break `HealthChangeIngestionStore.getIntervalSource`, which `HealthChangeSynchronizerImpl` and `WorkoutEnrichmentRefresher` use to resolve a distance/elevation record's *previous* range when Health Connect reports an update or delete — losing it means a corrected or deleted workout distance silently never marks its dates dirty. Because it is an allowlist, a future record type defaults to kept, never to collectable. **(2) Unreferenced:** zero rows in `heart_rate_records`, zero in `hrv_records`, zero in `hr_source_minute_contributions`, and named by neither in-flight staging table (`staged_hr_sources`, `scan_seen_ids`) — the same identity a backup export walks, so this predicate is exactly what keeps an export FK-complete. Deliberately conservative (any doubt keeps the row); `heart_rate_records`/`hrv_records` also carry `ON DELETE CASCADE` and `hr_source_minute_contributions` carries `ON DELETE RESTRICT` back to `health_source_records.id` as a schema-level backstop if the predicate were ever wrong. Collected counts are logged (counts only, no identifiers) but never fold into `totalDeleted`/the returned `AffectedRange`. Runs inside the same `HealthMutationCoordinator` mutation the rest of `deleteBefore` holds, which is the same coordinator sync/resync (`HealthSyncUseCase`) and backup export/restore acquire before touching Room — so GC can never observe a staging row or backup read mid-write from those flows. |
| `ScoreInvalidation`           | `core/model/src/main/kotlin/app/readylytics/health/core/model/domain/sync/ScoreInvalidation.kt`             | Pure Kotlin, zero Android dependencies. `AffectedRange(start, endInclusive)` plus `affectedRange(changed, today)`, which widens a touched range forward by `MAX_DEPENDENT_WINDOW_DAYS` (84 — the longest scoring lookback: the 84-day TRIMP fetch window) and caps the result at `today`. No longer used by the aging workers (data aging out never invalidates retained summaries; see `DataRollupWorker`/`DataCleanupWorker`, above); kept as the bound for any bounded recompute request via `WorkerScheduler.scheduleResyncWorker`'s `startDate`/`endDate`. A depth-guard test enumerates every scoring lookback constant (`ACUTE_DAYS`, `CHRONIC_DAYS`, `BASELINE_DAYS`, `HRV_SIGMA_WINDOW_DAYS`, `CIRCADIAN_CONSISTENCY_WINDOW_DAYS`, `MATURE_DATA_TENURE_DAYS`, the 84-day TRIMP fetch) and fails the build if any exceeds `MAX_DEPENDENT_WINDOW_DAYS`. **Task 5:** also `exampleFanOutRange(correctionDate, today, retentionStart)` — a second, additive dependency (`EXAMPLE_SELECTION_LOOKBACK_DAYS` = 30, its own depth-guard against `MAX_DEPENDENT_WINDOW_DAYS`) for recommendation-example eligibility (§2.11.3/§1.2's correction-fan-out paragraph), bounding `[correctionDate, correctionDate + 30]` to `[retentionStart, today]`. Wired into `DailySyncUseCase.resolveInlineOldestTargetDay`, merged alongside the naive out-of-window-affected-date floor. |
...
| `HealthChangeSynchronizer`    | `core/healthconnect/src/main/kotlin/app/readylytics/health/core/healthconnect/domain/sync/HealthChangeSynchronizer.kt`    | Reconciles differential Health Connect Changes API responses (upsertions and deletions) incrementally during daily/foreground sync. As of R2-ARCH-002, this path shares the same persistence boundary as the bulk/resync path (`HealthIngestionCoordinator`), split across two ports to keep both under detekt's function-count limit — `HealthChangeSynchronizerImpl` holds no DAO reference and no `Context`; it persists via `HealthIngestionStore` (`persist`/`replaceHeartRateSources`/`replaceHrvSources`) and resolves per-record delete/date-lookup via the new `HealthChangeIngestionStore.affectedDatesForRecord`/`deleteRecord`, session linkage via `HealthChangeIngestionStore.sessionSpansOverlapping` (hoisted once per changes page, R2-HC-003), and provisional workout metrics via `HealthChangeIngestionStore.heartRateSamplesForMetrics`. HR/HRV/exercise upserts resolve real overlapping sleep/workout session spans and, for exercise, real stored HR samples, so a changes-path row is link/metric-correct at write time rather than only after the next `DailySyncUseCase` reconcile pass (HC-004). For an upsertion from a non-selected device, it reads the stored record's affected dates before deleting that record for every data type, including `EXERCISE`; this removes an excluded workout and its route rows through Room's delete cascade. A matching `EXERCISE` upsertion instead uses `persistPreparedWorkouts` without pre-delete, preserving `modelTrimp` and the coalesced route, distance, and elevation state. `DeletionChange` continues to delete every type. The synchronizer returns candidate next tokens but never persists them; the daily-sync flow (`DailySyncUseCase`) commits them only after requested-window ingest, reconciliation, step fetch, and scoring all succeed. Changes older than the requested scoring window leave tokens uncommitted and return `REQUIRES_HISTORICAL_RESYNC`, routing correction through the durable worker without widening foreground scoring. **H3/WP-08:** `HealthDataType.VO2_MAX` participates in this same generic per-type loop (`recordClassesFor` maps it to `Vo2MaxRecord::class`, `upsertRecord` dispatches to `upsertVo2Max`) with no special-casing in `applyPendingChanges`/`captureChangesTokens`. Unlike the composite-id-keyed vitals (weight/body fat/blood pressure/SpO2/body temperature, `id = "${hcId}_${timestampMs}"`), VO2 max keeps its raw stable HC id as the Room primary key, so `RoomHealthChangeIngestionStore.affectedDatesForRecord`/`deleteFromDaos` resolve and delete it via a direct `Vo2MaxRecordDao.getById`/`deleteById` lookup rather than the `getBySourceRecordId` prefix-range pattern; the pre-delete timestamp read (before `deleteById`) is what feeds the existing P2 dirty-range journal (`deleteRecordAndJournal`), unchanged. **H4/WP-08 (OD-4 gate):** Tracks distance (`DISTANCE`) and elevation gained (`ELEVATION_GAINED`) interval changes via independent ingestion tokens (`IngestionTokenType`), bootstrapping tokens on first authorized run without resync, staging candidate tokens, and delegating workout enrichment refreshes to `WorkoutEnrichmentRefresher` outside transactions with atomic persistence via `persistIntervalEnrichment`. |
| `HealthChangeTokenStore`      | `core/model/src/main/kotlin/app/readylytics/health/core/model/domain/sync/HealthChangeTokenStore.kt`      | Atomically stores differential Changes Tokens per Health Connect data type after derived summaries are durable. Supports token suspension (`suspendType`, `isSuspended`) on permission denial/revocation, which clears the revoked token and flags the type as suspended to avoid full-resync loops while preserving existing records until re-grant. Because a cleared token forces a full historical resync on re-grant, `HealthChangeSynchronizerImpl` suspends a type **only** when its read permission is absent from `getGrantedPermissions()`. A `SecurityException` from `getChanges` for a type that is still listed as granted (e.g. Health Connect refusing a read while the app is backgrounded) skips that type for the run and keeps its token. A failed `getGrantedPermissions()` lookup fails the whole sync (caller retries) instead of reading as "nothing granted", which used to suspend every token and turn the next sync into a full resync. Replayed pages are safe because Room ingestion is idempotent by HC record ID. |
| `ResyncCheckpointStore`       | `core/model/src/main/kotlin/app/readylytics/health/core/model/domain/sync/ResyncCheckpointStore.kt`       | Stores the resumable resync checkpoint: fixed range, current phase (`INGEST` / `PRUNE` / `RECONCILE` / `RECOMPUTE`), next date, selection identity, token map, optional `chunkDaysOverride` (HC-002: shrunk ingest chunk size), nullable `hrPageToken`/`hrvPageToken` (R2-HC-002: page tokens for resuming streamed sample reads mid-chunk), `completedTypes` (WP-06: set of health data types that completed ingestion and deletion reconciliation across chunks), `completedTypesRecorded` (H1 review fix), and `runIdentity: HistoricalRunIdentity?` (WP-10: protocolVersion = 3 immutable snapshot binding the run to mode, date range, startedAtEpochMs, scoring zone, retention boundary, source selection, algorithm revision, scoringSnapshotJson, and scoringSnapshotId). Matching scoring snapshots resume; changed scoring inputs, retention settings, or older protocols restart recompute at the requested range start. Full resyncs require pre-ingest baseline Changes Tokens and clear only after recompute succeeds and tokens are promoted; local recomputes intentionally persist an empty token map and clear after successful scoring. |
| `ResyncCheckpointStoreImpl`   | `app/src/main/kotlin/app/readylytics/health/data/preferences/ResyncCheckpointStoreImpl.kt` | Proto DataStore implementation of historical-resync checkpoint persistence. `chunk_days_override` (proto field 7, additive) round-trips as `chunkDaysOverride: Int?` — proto `0` (the proto3 default) decodes to domain `null`/"no override"; `hr_page_token` (proto field 8) and `hrv_page_token` (proto field 9) round-trip as nullable strings (blank/unset decodes to `null`, R2-HC-002); `completed_types` (proto field 10, repeated string) round-trips as `completedTypes: Set<HealthDataType>`; `completed_types_recorded` (proto field 11, bool, H1 review fix) round-trips as `completedTypesRecorded: Boolean`; `run_identity` (proto field 12, WP-10) round-trips as `runIdentity: HistoricalRunIdentity?`, encoding/decoding protocol version, run ID, mode, epoch days, zone ID, start timestamp, source selection ID, algorithm revision, scoring snapshot JSON, and scoring snapshot ID. Legacy checkpoints lacking field 12 decode `runIdentity` as `null` and safely restart. |
...
| `SelectedSourcePrunerImpl`    | `core/database/src/main/kotlin/app/readylytics/health/core/database/data/local/SelectedSourcePrunerImpl.kt`     | Concrete implementation of `SelectedSourcePruner`. Deletes non-selected-device records within scoring-zone-derived epoch boundaries; ambient device timezone never changes which edge-day records are pruned. `HealthDataType.VO2_MAX` (H3/WP-08) fetches candidates via `Vo2MaxRecordDao.getByTimeRange`, filters by device name in Kotlin, then deletes non-matching rows individually via `deleteById` in a loop (distinct from sibling single-table vitals types which use bulk `deleteRecordsNotMatchingDevice` SQL deletes; VO2 DAO omits the bulk method); `STEPS` remains the sole no-op (its daily totals live in `daily_summaries`, not a per-record table). |
...
| `SessionLinkReconciler` | `core/model/src/main/kotlin/app/readylytics/health/core/model/domain/sync/link/SessionLinkReconciler.kt` | Domain-side reconcile port. Data implementation (`core/database/src/main/kotlin/app/readylytics/health/core/database/data/local/SessionLinkReconcilerImpl.kt`) runs post-ingestion **once per `resyncRange` call** (not per chunk): loads the complete sleep + workout session spans for `[start - 1 day, end]`, builds one `SessionLinkSweep` per HR/HRV pass, re-tags every row in range via `sweep.resolve(timestampMs)`, and recomputes `trimp`/zone-minutes/`avgHr`/`durationMinutes` for every workout in range via `ZoneThresholds.computeMetrics` (`core/model/src/main/kotlin/app/readylytics/health/core/model/domain/heartrate/ZoneThresholds.kt`, extracted from the old `WorkoutMapper` as a pure domain calculator). Processes heart rate and HRV records in keyset-paged chunks of 5000 records, running each batch in a transaction. **WP-17 Step 4:** between the raw HR/HRV passes and the workout recompute it runs `WarmTierRelinker.relink(start, end, sleepSpans, workoutSpans)` once over the same complete range, which re-keys warm minutes whose session assignment changed and retires minutes whose per-source evidence a Health Connect deletion removed (see "Warm links are re-derived, not re-tagged" in §1), then logs the returned `WarmRelinkOutcome`'s five counts at `logI("SessionLinkReconciler")` — the only production observation point for how much history is still legacy/approximate or carries unreadable evidence. It must run before `recomputeWorkouts`, which reads the relinked warm projection through `AuthoritativeHeartRateReader`. Workout recomputation (`recomputeWorkouts`, HC-002) is batched in groups of `WORKOUT_BATCH_SIZE` (20) by `workoutSpans` order (startTime ASC, from `WorkoutDao.getOverlapping`): each batch fetches its own workout rows via `WorkoutDao.getByIds`, fetches EXERCISE-type HR samples spanning the whole batch's time range via `AuthoritativeHeartRateReader.rawByTypeInRange` (DB-001, index-backed, no in-memory type filtering) plus each workout's own visible warm slices via `AuthoritativeHeartRateReader.warmSessionSamples` (WP-17 Step 3 — so a workout straddling the hot/warm cutoff contributes each of its minutes exactly once, and never twice), computes metrics per workout via `ZoneThresholds.computeMetrics` (which internally clips `hrSamples` to each workout's own `[startMs, endMs]`, so sharing the batch's HR list across its workouts is safe), and upserts the whole batch in one transaction — cutting both DB reads and transaction count from O(workouts) to O(workouts / 20). Only changed rows are upserted. |
...
> `HealthSyncUseCase` and share the application-scoped `HealthMutationCoordinator`; ingestion stays upsert-by-HC-id, so overlapping
...
| `HealthChangeVitalsInputMappers` | `core/healthconnect/src/main/kotlin/app/readylytics/health/core/healthconnect/data/healthconnect/HealthChangeVitalsInputMappers.kt` | `DomainWeightRecord`/`DomainBodyFatRecord`/`DomainBloodPressureRecord`/`DomainOxygenSaturationRecord`/`DomainBodyTemperatureRecord` → their `*Input` types, on the Changes API path (`toWeightInput`, `toBodyFatInput`, `toBloodPressureInput`, `toOxygenSaturationInput`, `toBodyTemperatureInput`). |
| `VitalsInputMapper`          | `core/healthconnect/src/main/kotlin/app/readylytics/health/core/healthconnect/domain/sync/VitalsInputMapper.kt`          | Pure optional-vitals mapping (weight, body fat, blood pressure, SpO2, body temperature) before shared database persistence mappers apply entity writes. |
...
Version 20 (`Migration19To20`) bumps the schema for the Work Package 04/05 ingestion and recalculation foundation:
- Adds additive source provenance columns to `health_source_records`: `originPackage TEXT`, `recordStartMs INTEGER`, `recordEndExclusiveMs INTEGER`, `lastModifiedMs INTEGER`, `metadataState TEXT NOT NULL DEFAULT 'UNKNOWN'`, `sourceRevision INTEGER NOT NULL DEFAULT 0`. Adds index `index_health_source_records_recordType_metadataState_recordStartMs` on `(recordType, metadataState, recordStartMs)` to enable fast candidate selection and dirty-range bounding.
- Adds `dirty_ranges` table (`id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, sourceGeneration INTEGER NOT NULL, startEpochDay INTEGER NOT NULL, endEpochDayInclusive INTEGER NOT NULL, nextEpochDay INTEGER NOT NULL, reason TEXT NOT NULL, scoringSnapshotId TEXT NOT NULL` with index `index_dirty_ranges_sourceGeneration_id` on `(sourceGeneration, id)`). `RoomDirtyRangeStore.append` validates `startEpochDay <= endEpochDayInclusive` and `startEpochDay <= nextEpochDay <= endEpochDayInclusive + 1` before inserting; the entity can materialize malformed persisted rows without throwing. `DirtyRangeDao.discardBefore` first deletes rows violating those bounds, then deletes expired rows and trims retained prefixes in one transaction. The upper-bound check handles `Long.MAX_VALUE` without overflow. The Room schema version remains 23.
...
The same still-unreleased `Migration22To23` also carries two Phase 2 Task 10 indexes, added only
after `EXPLAIN QUERY PLAN` on the real production queries proved each necessary
(`Phase2QueryPlanTest`, see `benchmark/BASELINE.md`'s "Phase 2 — query plans and index cost"
section for the before/after plans and measured storage/write cost): `index_scan_seen_ids_sourceId`
on `scan_seen_ids(sourceId)` (the Task 9 GC's `runId`-less existence check, previously a bare table
scan) and `index_hr_v10_timestamp_source` on `heart_rate_records(timestampMs, sourceRecordRef)` (the
Task 8 rollup streamer's keyset page, previously a temp B-tree sort). Both are additive to this
still-unreleased v23, not a new migration version.
...
journal path.
**Independent distance/elevation interval correction and workout enrichment refresh (H4/WP-08, OD-4 gate).**
Distance (`DistanceRecord`) and elevation gain (`ElevationGainedRecord`) interval samples in Health Connect can be updated, moved, or deleted independently of their parent workout sessions.
`IngestionTokenType` defines 13 ingestion-only token types (11 display types + `DISTANCE` + `ELEVATION_GAINED`), decoupling ingestion change-tracking tokens from UI-facing `HealthDataType`.
`HealthChangeSynchronizerImpl.applyPendingChanges` processes changes for `DISTANCE` and `ELEVATION_GAINED` through their own separate changes tokens without forcing full historical resync. On the first authorized run for interval types, tokens are bootstrapped to `ChangesTokenStatus.SUCCESS` with the initial token. Next candidate tokens are staged in memory (`stagedIntervalTokens`) and promoted in `HealthChangeTokenStore` only upon `commitTokens()`.
Each interval change (`IntervalChange`) captures `sourceId`, `kind`, `oldStartMs`, `oldEndExclusiveMs`, `newStartMs`, `newEndExclusiveMs`, and `originPackage`.
Known records track their exact bounds in `health_source_records` via `IntervalSourceRecord`. For deletions of interval records without previously stored bounds (such as legacy records predating interval tracking or unmapped deletions), `WorkoutEnrichmentRefresher` logs a warning and skips unmapped total clearance to avoid wiping unassociated historical workouts, deferring reconciliation to full historical resync.
When changes occur, `WorkoutEnrichmentRefresher` finds overlapping workouts via `HealthChangeIngestionStore.workoutsOverlapping` using half-open intervals (`overlaps(aStart, aEnd, bStart, bEnd) = aStart < bEnd && bStart < aEnd`).
Crucially, all Health Connect SDK reads (`readRecord` for workout session origin attribution, `readRecords` for distance and elevation gained) execute outside database transactions in bounded chunks.
Attribution is preserved: interval samples are filtered to match the workout session's authoritative data origin package (falling back to device name).
Results are merged into `PreparedWorkout` using H5's `mergeEnrichment` rules and atomically committed via `HealthChangeIngestionStore.persistIntervalEnrichment`, which updates `workout_records`, journals dirty ranges in `dirty_date_ranges` for walk-forward recompute, and updates or removes metadata in `health_source_records` within a single Room transaction.
...
- **Warm tier (90 days → retention cutoff):** 1-minute `hr_minute_buckets` per
  ...
  `DataRollupManager.rollupExpiredHotTier(cutoffMs)`, which downsamples raw HR older than the
  boundary into buckets and deletes the raw rows. **PERF-003 (Task 8):** within a day chunk this is
  no longer one read-aggregate-publish-delete transaction — `MinuteRollupStreamer` streams keyset
  pages outside any transaction into bounded groups of complete minutes, aggregates each group in
  Kotlin outside any transaction, then opens one short transaction per group to publish + delete
  exactly that group. A crash can still never drop a sample: either the raw row survives (its group
  never committed) or it is already folded into a published bucket (its group's transaction
  committed both the bucket and the delete atomically) — the granularity moved from one day to one
  bounded group, the guarantee did not. `ScoringRepositoryImpl` merges hot+warm
...
**One tier and one generation per minute (WP-17 Step 3).** Every heart-rate reader selects its
tier through `AuthoritativeHeartRateReader`
(`core/database/src/main/kotlin/app/readylytics/health/core/database/data/local/AuthoritativeHeartRateReader.kt`),
the single owner of the `minute_coverage` visibility predicate. Rules, identical for means,
percentile projections, charts, workout HR and baseline coverage/count checks: a minute with no
coverage row is raw-only; a minute with `HOT` coverage is raw-only and hides its older warm
projections; a minute with `WARM`/`LEGACY_WARM` coverage is warm-only and only the bucket slices at
that row's `visibleGeneration` are visible, so a superseded slice left behind by a crash stays
invisible. An overlap is never summed or concatenated — it resolves to exactly one tier. This
matters because the OD-1 quarantine rule (see `DataRollupManager`, below) deliberately keeps a
legacy minute's raw rows *and* its approximate warm projection; before the predicate every reader
added the two together and inflated that minute's sample count and mean. The raw-side predicate
keys minutes with `((t / 60000) - CASE WHEN t % 60000 < 0 THEN 1 ELSE 0 END) * 60000`, the SQL
equivalent of `completeMinuteCutoff`'s `floorDiv`, so a pre-1970 sample lands in the same bucket as
in the ledger. `minute_coverage.bucketStartMs` is a single-column INTEGER primary key (a rowid
alias), so the join is a rowid seek and needs no added index; the warm-side fallback subquery is
covered by the existing `(bucketStartMs, bucketEndMs)` index. Both claims are asserted against real
SQLite by `AuthoritativeHeartRateReaderEquivalenceTest`'s `EXPLAIN QUERY PLAN` case, which fails on a
table scan. Consumers: `HeartRateRepositoryImpl`, `ScoringHistoryRepositoryImpl`,
`ScoringHeartRateDataLoader` and `SessionLinkReconcilerImpl.recomputeWorkouts`. Enforced by
`AuthoritativeHeartRateReaderEquivalenceTest` (cross-reader count/mean equivalence against a
reference decoded from the stored contribution histograms) and, on real on-device SQLite, by
`AuthoritativeHeartRateReaderTest`.

The two predicates are a **total** partition of the minute space, not merely a disjoint pair. The one
hole — coverage committed as `WARM`/`LEGACY_WARM` at a generation with no bucket slice, reachable only
from a partially applied restore or a future writer that commits coverage without its projection — is
closed on the raw side: `HeartRateDao`'s predicate carries a
`NOT EXISTS (… hr_minute_buckets b2 WHERE b2.bucketStartMs = c.bucketStartMs AND b2.generation =
c.visibleGeneration)` last-resort fallback, so such a minute degrades to showing its raw evidence
...
`rawSamples` alone. `mergedSamples()` reconstructs each visible warm bucket into raw-shaped rows that
keep **that bucket's own** `recordType`/`sessionId`/`deviceName`
(`List<HrMinuteBucketEntity>.reconstructAsRecords`), so downstream filters such as
`ComputeSleepMetricsUseCase`'s `recordType == SLEEP && sessionId in currentSessionIds` still select
correctly. This is load-bearing on a scoring path: `ScoringHistoryRepositoryImpl
.getHeartRateRecordsByTimeRange` feeds `HrCoverageValidator.isValid`, which fails an empty list, so a
raw-only read would withhold the sleep score of any night whose minutes have rolled up or been
...
(`core/database/src/main/kotlin/.../data/local/WarmTierRelinker.kt`) runs once over the complete
range from `SessionLinkReconcilerImpl.reconcile`, between the raw HR/HRV passes and the workout
recompute. Per `SOURCE_BACKED` minute it reconstructs each contribution's evidence from its
immutable `bpmHistogram` plus `firstSampleMs`/`lastSampleMs`
(`HrSourceMinuteContributionEntity.reconstructEvidence`), resolves every reconstructed sample
through the unchanged `SessionLinker` (sleep > workout > resting, tiebreak `(startTime, id)`), and
re-aggregates through the rollup's own `MinuteBucketAggregator` — then republishes only the minutes
...
it (measured on the `WarmTierRelinkTest` fixture: worst count delta 1 sample, worst weighted-mean
delta 0.50 bpm; totals are conserved exactly); (2) `LEGACY_UNKNOWN` minutes have no per-source
evidence at all, so they are left entirely untouched and only *counted*
(`WarmRelinkOutcome.unresolvedLegacy`, `AuthoritativeHeartRateReader.legacyApproximateMinutes`) —
publishing a "corrected" canonical score on guessed lineage is what OD-1 forbids, and only an
authorized complete interval refresh can repair them. **That refresh is not implemented**: no
production path replaces a `LEGACY_UNKNOWN` minute today (`HeartRateRefreshStagingStore` has no
production caller, and the Settings "Resync Health Connect data" flow re-ingests raw HR but the
rollup quarantines those minutes and this pass skips them), which is why `ABOUT.md`/`docs/about.md`
state the gap rather than promising a repair. `SessionLinkReconcilerImpl` logs the whole
`WarmRelinkOutcome` (`inspected`/`republished`/`retired`/`unresolvedLegacy`/`unresolvedEvidence`) once
per reconcile at `logI("SessionLinkReconciler")`, so how much history is still approximate — and any
corrupt-histogram minute — is observable on a real device run, not only in tests. The same pass is how a Health Connect
deletion of an already-rolled-up source converges: `SourceRecordDao.deleteBySourceRecordId` drops
that source's contributions and cascades its raw rows, and the next reconcile rebuilds each
affected minute from the remaining contributions or retires a minute whose evidence is gone
(deleting its buckets and its coverage row together, in one transaction).
...
| `MinuteCoverageEntity`         | `minute_coverage`           | `bucketStartMs`                        | `visibleGeneration`, `tier`, `quality`, `sourceSelectionId`                                                                                               |
| `HrSourceMinuteContributionEntity`| `hr_source_minute_contributions` | `(sourceRecordRef, bucketStartMs, generation)` | `firstSampleMs`, `lastSampleMs`, `deviceName`, `bpmHistogram`                                                                                              |
| `StagedSourceMetadataEntity`   | `staged_hr_sources`         | `(runId, sourceId)`                    | `recordType`, `originPackage`, `startMs`, `endExclusiveMs`, `lastModifiedMs`, `payloadComplete`                                                           |
| `StagedHeartRateEntity`        | `staged_hr_samples`         | `(runId, sourceId, timestampMs)`       | `beatsPerMinute`, `recordType`, `sessionId`, `deviceName`                                                                                                 |
| `HrMinuteBucketEntity`         | `hr_minute_buckets`         | `(bucketStartMs, recordType, sessionId, deviceName)` | 1-minute warm-tier aggregates: `minBpm`/`maxBpm`/`avgBpm`/`sampleCount`; `sessionId` is `""` for no-session minutes; `deviceName` (default `""`); plus a p5/p25/p50/p75/p95 percentile sketch (nullable — `null` for buckets rolled up before the v15 migration); `generation` (default `0`) |
...
**Encrypted Snapshot Export Pipeline (WP-16 / SEC-003/005):**
Local backup generation produces a fully encrypted, point-in-time snapshot with zero intermediate plaintext files:
1. **P2 Maintenance Gate Hold:** `BackupSnapshotExporter.captureEncrypted` acquires and holds the P2 maintenance owner (`HealthMutationCoordinator.withMaintenance(operationId)`) throughout snapshot capture. This freezes the database generation and blocks competing mutations, syncs, cleanups, and rollups from altering health tables mid-stream.
2. **Atomic Admission Identity:** User preferences, active layout configurations (dashboard cards, sleep cards, and workout cards), and `BackupSnapshotIdentity(sourceGeneration, scoringSnapshotId, schemaVersion, exportedAtEpochMs)` are captured once at admission under the coordinator lock before streaming begins.
3. **Direct AES-256 ZIP Streaming:** The database JSON is streamed directly through `BackupStreamWriter.writeJsonStreaming` into a Zip4j `ZipOutputStream` using AES-256 encryption. Plaintext JSON is never written to disk at any point during staging or export.
4. **Keyset Paging for Source Records:** To avoid loading large source record tables into memory, `BackupStreamWriter.writeCoreTables` streams `health_source_records` in bounded chunks using keyset pagination (`SourceRecordDao.pageAfter(afterRef, limit)`).
5. **WP-17 Coverage Tables (archive schema v22):** `CoverageBackupWriter` streams `minute_coverage` and `hr_source_minute_contributions` alongside the warm buckets, carrying source FKs, `generation` and `quality` so a restored archive keeps the same visible generation per minute. Contributions page on their FULL composite primary key `(bucketStartMs, sourceRecordRef, generation)` — `bucketStartMs` is not unique there, so a single-column cursor would silently drop every remaining row of a group whose page boundary fell inside it and then fail the manifest row-count check. Staged refresh work (`staged_hr_sources`/`staged_hr_samples`) is local operational state and is deliberately NOT exported. Restore order is sources → contributions/coverage/projections; clearing runs contributions before sources because contributions reference `health_source_records` with `ON DELETE RESTRICT`. Archives older than v22 are initialized by `CoverageRestoreLoader` as `LEGACY_WARM`/`LEGACY_UNKNOWN` at generation 0 — the same state migration 21→22 produces — so restored legacy minutes stay pending an authorized refresh and tiers are never mixed by concatenation.
5. **Private Staging & Safe Lifecycle:** The encrypted archive is staged directly into a private app-owned staging directory (`cacheDir/backup-staging/<operationId>.zip`). On any error or cancellation, the incomplete staging file is deleted immediately and database maintenance state is reset (`setMaintenance(null, null)`). Once encrypted staging completes successfully, the maintenance gate is released before publishing the encrypted archive to the external SAF destination. Staging orphans from crashed process runs are pruned on startup by `CachePrune` / `DatabaseReadyStartupInitializer`.
...
**Two-Resource Restore Coordination & Startup Recovery (WP-16 / ARCH-001):**
Room SQLite database and DataStore preferences are separate atomic resources. To prevent mixed-generation states (e.g. newly restored database scored against stale pre-restore preferences or premature recommendation backfills running with wrong thresholds), `LocalRestoreManager` delegates execution to `RestoreMaintenanceCoordinator`:
1. **Maintenance Gate & Durable Journal:** `RestoreMaintenanceCoordinator` acquires the P2 maintenance gate (`HealthMutationCoordinator.withMaintenance(operationId)`). Before any database modification occurs, it serializes and encrypts the restore payload identity, restored generation ID, preferences snapshot, and credentials into an external encrypted journal file (`RestoreOperationJournal`, `filesDir/restore_operation_journal.enc`) via `EncryptionManager`, recording phase `VALIDATED`.
...
6. **Startup Recovery:** On application launch, `DatabaseReadyStartupInitializer` runs `RestoreMaintenanceCoordinator.recoverInterruptedRestoreOnStartup()` before historical baseline backfill or background workers are admitted. If an interrupted restore is detected at `DATABASE_COMMITTED` (either in SQLite or journal), preferences and layouts are replayed from the encrypted journal, tokens and checkpoints are reset, recompute is scheduled, and maintenance is released. If an interruption occurred prior to database commit (`VALIDATED`), the orphaned journal and maintenance marker are cleared, leaving the original database and preferences intact. Concurrent mutations enter `HealthMutationCoordinator.withMutation`; the coordinator checks the durable maintenance marker under the same mutex and rejects them while recovery is pending.
...
In addition, `RestoreDatabaseOperations.regenerateRestoredDirtyWork` regenerates a dirty range spanning all retained dates from the earliest to the latest summary date, ensuring recalculation runs cleanly over restored history.
...
To guarantee safe concurrency between foreground daily sync, background historical resync, data retention cleanup, warm-tier rollup, and backup/restore, the application uses one application-scoped mutation owner with a strict, unidirectional lock hierarchy:
```
HealthMutationCoordinator -> calculationMutex -> Room transaction
```
1. **`HealthMutationCoordinator` (`withMutation`, `withMaintenance`)**: Application-scoped owner backed by a coroutine `Mutex`. All mutation entry points (Health Connect ingestion, retention cleanup, data rollup, startup backfill, restore, and backup snapshot capture) acquire this coordinator. Before admitting any ordinary mutation, it inspects durable state (`stateDao.current().maintenanceOperationId == null`). If a durable maintenance operation is pending or running, ordinary mutations fail fast. `withMaintenance(operationId)` records and validates the operation ID in `health_mutation_state` under the coordinator lock and only clears durable state upon verified completion. Cancellation or failures release in-memory locks while leaving durable recovery state intact. The coordinator supports reentrancy for the owning coroutine (`MutationOwner` marker in coroutine context matching this coordinator instance and current `Job`), enabling nested `withMutation` calls without deadlocking, while non-owning coroutines (including child jobs) serialize against the mutex.
2. **`calculationMutex`**: Guards daily scoring computations in `ScoringRepositoryImpl`, preventing concurrent score derivations for overlapping days.
3. **Room Transactions**: Leaf SQLite transactions executed via `RoomTransactionRunner` or `SupportSQLiteDatabase.runInTransaction`.

**Settings Recompute Serialization & Maintenance Deferral (WP-07):**
Interactive settings adjustments that affect the current day's scoring (`SleepSettingsViewModel`, `ThresholdSettingsViewModel`, `UserUseCase.updateBirthday`) must not run concurrently with active ingestion, historical resync, or maintenance operations. Rather than calling `ScoringRepository.computeAndPersistDailySummary(today)` directly, all settings call sites route through the `RecomputeToday` interface (`RecomputeTodayUseCase`). `RecomputeTodayUseCase` executes within `HealthMutationCoordinator.withMutation`. If maintenance is pending, it catches the maintenance condition, schedules a bounded one-day resync worker via `WorkerScheduler.scheduleResyncWorker(recomputeOnly = true, startDate = day, endDate = day, trigger = RecalcTrigger.SETTINGS_CHANGE)`, and returns `Result.Failure(code = "MAINTENANCE_PENDING")`. Cancellation propagates without scheduling or swallowing. For birthday updates in `UserUseCase`, preferences are written in strict sequence: birthday is persisted, automatic max HR is updated if enabled, today is recomputed via `RecomputeToday.execute(today)`, and finally the durable historical recompute pass is enqueued.
...
1. **Atomic Mutation Boundary (Ingestion & Maintenance):**
   - Whenever raw records are deleted or upserted by Health Connect ingest, old and new date extents are resolved using the stored scoring timezone. Data *aging* (retention cleanup, hot→warm rollup) is deliberately excluded: it never journals dirty work, because retained summaries are frozen snapshots computed while the aged data still existed. Tickets journaled with the retired `HOT_TIER_ROLLUP`/`RETENTION_CLEANUP` reasons by older builds are dropped by `DirtyRangeStore.discardRetiredAgingTickets()` at startup and in `HealthResyncWorker`.
   - Inside a single atomic Room transaction, raw data mutations are applied to SQLite, `health_mutation_state.sourceGeneration` is incremented (a data revision counter, not a timestamp; identical replays do not increment it), and dirty date extents (`[earliestAffectedDay, today]`) are appended to `dirty_ranges`. **PERF-003 (Task 8) exception:** `DataRollupManager`'s hot→warm rollup no longer increments the generation and mutates SQLite inside one transaction per day — `MinuteRollupStreamer` streams and groups outside any transaction, `nextGeneration()` is captured once per bounded group, and only that group's own publish + delete run inside a transaction, which re-validates the captured generation before writing. This keeps the same "generation bump and its data mutation commit together" guarantee at group granularity instead of day granularity, while keeping CPU-heavy percentile/histogram aggregation out of the writer transaction entirely (PERF-003).
   - WorkManager enqueue never precedes the database commit. If process death or crash occurs before enqueue, `DatabaseReadyStartupInitializer` queries `dirtyRangeStore.pending(100)` at startup and enqueues a recompute-only pass (`RecalcTrigger.STARTUP_PENDING_DIRTY`, detail listing each ticket's `reason:next..end`), closing the crash gap. The daily sync drains tickets inside its inline reach (see `DailySyncUseCase`), so this gate should only fire for genuinely missed work.
...
2. **Atomic Publication Boundary (Scoring & Acknowledgment):**
   ...
   - If a concurrent mutation arrived during computation (`liveGeneration != expectedSourceGeneration`), the publisher rejects acknowledgment and rolls back the transaction, returning `false`. The unacknowledged dirty range remains intact in SQLite, guaranteeing that the newer revision is recomputed without losing historical invalidation state.
    - A successful `Absent` publication advances the dirty ticket cursor exactly like `Computed` (both are genuinely complete candidates). A failed publish or an `Unavailable` assembly never advances it. Of the four walk-forward contexts: `WalkForwardContexts.trimp` is mutated per-day by this pipeline (`DailyTrimpComputer.publishTrimpToContext`), and that mutation is applied by the caller only *after* a successful write (`commitWalkForwardContexts()` strictly after `persistDayAssembly` returns `true`). `WalkForwardContexts.fatigue` (WP-22) uses dual-cursor preview/commit semantics (`morningCursor` and `dayEndCursor`): morning residual fatigue evaluates provisional state via `morningCursor.previewThrough(wakeTimeMs)` without committing in-place, and day-end evaluates via `dayEndCursor.previewThrough(nextDayMidnightMs)`; both cursors are committed strictly *after* `persistDayAssembly` succeeds and the 1-day publication transaction commits, preventing failed or `Unavailable` assemblies from contaminating accumulator state. `WalkForwardRasWindow` tracks a rolling 6-day window of prior daily summaries seeded at run start, appending the newly committed summary and evicting the oldest strictly after `persistDayAssembly` succeeds. `baseline` and `vo2Max` are populated once up front per walk-forward run (`fetchWalkForwardBaselineContext`/`fetchWalkForwardVo2MaxContext`), with `baseline` memoizing per-night projections by `NightCacheKey(sessionId, sourceGeneration, scoringSnapshotId)`.
...
`WalkForwardVo2MaxContext` (`TreeMap<Vo2MaxKey, Float>` keyed by `Vo2MaxKey(timestampMs, id)`), instead of every
recomputed day issuing its own `Vo2MaxRecordDao.getLatestInRange` point query; each day looks up
`lowerEntry(Vo2MaxKey(nextDayMidnightMs, ""))` and checks that entry's timestamp against its own 30-day lookback
floor — reproducing the exact exclusive upper bound `[minTimestampMs, endExclusiveMs)` and deterministic tiebreak
(`ORDER BY timestampMs DESC, id DESC`) semantics the per-day query uses. The single-day
overload (context null) queries `BodyMetricsDataLoader.loadLatestVo2Max(wearableLookbackMs, nextDayMidnightMs)`.
...
`ComputeHistoricalBaselinesUseCase`) runs at app start under `HealthMutationCoordinator.withMutation`
...
**Run-owned night caching (WP-22):** `WalkForwardBaselineContext` maintains an in-memory run-owned cache of nightly aggregations (`NightCacheKey(sessionId, sourceGeneration, scoringSnapshotId)`) memoizing ordered BPM (`List<Int>`) and average BPM per sleep session. Because percentiles and restoration nadirs only depend on sleep HR samples, this memoization eliminates repeated raw sample scans and expansions across overlapping baseline lookback windows while guaranteeing bit-identical equivalence to the reference implementation. The cache is bound to the immutable run identity and discarded on cancellation or retry.
...
**Dual-cursor preview and commit ordering (WP-22).** To eliminate fatigue state leakage on failed day assembly, `WalkForwardFatigueContext` manages two independent fatigue cursors: a morning cursor evaluated at wake timestamp (`morningCursor.previewThrough(wakeTimeMs)`) for morning recovery metrics, and a day-end cursor evaluated at next-day midnight (`dayEndCursor.previewThrough(nextDayMidnightMs)`). Canonical impulses are registered in a monotonic queue ordered by `(endTimeMs, workoutId)`. Previewing generates an immutable candidate accumulator state without altering the committed baseline. Only after `persistDayAssembly` commits the day's atomic 1-day transaction does `commitWalkForwardContexts` advance the cursors. If a day's calculation fails or degrades to `Unavailable`, the candidate is dropped and neither accumulator is advanced.
...
evaluates every workout it touches and persists `modelTrimpQuality` alongside `modelTrimp` (a value, including
`0f`, or `NULL` with quality `UNAVAILABLE` when the workout has no HR samples), and the gate counts only
never-evaluated rows (`modelTrimp IS NULL AND modelTrimpQuality IS NULL`), so the count reaches zero and the
gate stops firing. `UNAVAILABLE` rows are terminal — no recompute can fill them — so counting them would
re-enqueue a full retained-history recompute on every cold start. This also closes the matching
`COALESCE(modelTrimp, trimp)` inconsistency in ATL/CTL.
...
notification, exponential backoff, and the shared `HealthMutationCoordinator` ownership. The worker delegates to
`FullHistoricalResyncUseCase.executeTrainingReadinessProjection`, which retention-bounds the range and
then enters `HealthMutationCoordinator.withMutation` before calling `TrainingReadinessProjectionRecomputeUseCase`. That use case performs exactly one retained
...
| `UniversalMetricCard`                                                                          | `core/ui/src/main/kotlin/app/readylytics/health/core/ui/components/metriccard/UniversalMetricCard.kt`, `core/ui/src/main/kotlin/app/readylytics/health/core/ui/components/metriccard/UniversalMetricRenderers.kt` | Soft Arc Metric Card gauge layout (status-colored) with comparison delta pill (`UniversalGaugeRenderer` → `M3MetricGaugeWithValue`, `UniversalMetricDeltaPill`). |
| `UniversalMetricCard` / `MetricTooltip`                                                      | `core/ui/src/main/kotlin/app/readylytics/health/core/ui/components/metriccard/UniversalMetricCard.kt`, `core/ui/src/main/kotlin/app/readylytics/health/core/ui/components/MetricTooltip.kt` | Status-colored metric cards with tooltips. |
...
| `core/healthconnect/src/main/kotlin/app/readylytics/health/core/healthconnect/domain/sync/HealthSyncUseCase.kt`                                         | Ingestion — sync facade (uses `HealthMutationCoordinator.withMutation`)          | delegates `sync` → `DailySyncUseCase`, `resyncRange` → `ResyncRangeUseCase`; no local lock              |
...
| `core/database/src/main/kotlin/app/readylytics/health/core/database/domain/sync/DailyRecomputeSupport.kt`                                     | Ingestion — per-day recompute helpers               | `recomputeDay` → `ScoringRepository` (no math) + `refreshAutoMaxHr`                     |
...
| `core/model/src/main/kotlin/app/readylytics/health/core/model/domain/repository/ReadOutcome.kt`                                       | Ingestion — read outcome wrapper                    | Models Available, Denied, and Unsupported Health Connect reads                          |
...
| `app/src/main/proto/resync_checkpoint.proto`                                            | Ingestion — checkpoint schema                       | Resync phase/date; proto field 12 run_identity for protocol v3 immutable run snapshots  |
| `core/model/src/main/kotlin/app/readylytics/health/core/model/domain/sync/HistoricalRunIdentity.kt`                      | Ingestion — immutable run model                     | Protocol v3 immutable run identity binding mode, dates, instant, scoring zone, retention, and snapshot IDs          |
| `core/model/src/main/kotlin/app/readylytics/health/core/model/domain/sync/ScoringRunSnapshot.kt`                        | Ingestion — scoring preferences snapshot            | Canonical serialization parts 1-9 of preferences and source selection for run identity  |
| `core/model/src/main/kotlin/app/readylytics/health/core/model/domain/sync/ScoringRunContext.kt`                        | Ingestion — immutable scoring context               | One instant, scoring zone, today, and retention boundary reused for a complete run         |
| `core/healthconnect/src/main/kotlin/app/readylytics/health/core/healthconnect/domain/sync/HistoricalRunResolver.kt`       | Ingestion — run identity resolution                 | Resolves resume vs restart across runs, handling DST and midnight transitions          |
| `core/healthconnect/src/main/kotlin/app/readylytics/health/core/healthconnect/domain/sync/HistoricalIngestPhase.kt`       | Ingestion — chunked ingestion phase                 | Chunked HC window read, adaptive shrink, and deletion reconciliation                    |
| `core/healthconnect/src/main/kotlin/app/readylytics/health/core/healthconnect/domain/sync/HistoricalPrunePhase.kt`        | Ingestion — selected-source prune phase             | Deletes already-ingested records whose device no longer matches the current selection   |
| `core/healthconnect/src/main/kotlin/app/readylytics/health/core/healthconnect/domain/sync/HistoricalRecomputePhase.kt`    | Ingestion — walk-forward recompute phase            | 1-day unit Room transactional walk-forward recompute with 30-day checkpoint commits      |
...
| `core/model/src/main/kotlin/app/readylytics/health/core/model/domain/sync/IngestionTokenType.kt`                             | Domain — ingestion token registry (H4/WP-08)         | 13 types: 11 display types + `DISTANCE` + `ELEVATION_GAINED`, decoupling sync tokens from display types |
| `core/model/src/main/kotlin/app/readylytics/health/core/model/domain/sync/IntervalChange.kt`                                 | Domain — interval change & half-open overlap (H4)   | `IntervalChange(sourceId, kind, oldStartMs, oldEndExclusiveMs, newStartMs, newEndExclusiveMs, originPackage)`; `overlaps()` |
| `core/model/src/main/kotlin/app/readylytics/health/core/model/domain/sync/IntervalSourceRecord.kt`                           | Domain — interval source record model (H4)          | Tracks distance/elevation interval bounds & origin in `health_source_records`                            |
| `core/healthconnect/src/main/kotlin/app/readylytics/health/core/healthconnect/data/healthconnect/WorkoutEnrichmentRefresher.kt`| Ingestion — interval enrichment refresher (H4/WP-08)  | Queries overlapping workouts, reads distance/elevation by origin outside transactions, atomically applies H5 enrichment |
...
| `core/model/src/main/kotlin/app/readylytics/health/core/model/domain/scoring/RecomputeToday.kt`                                           | Scoring — single-day settings recompute port        | `execute(day: LocalDate): Result<Unit>`                                                  |
| `core/database/src/main/kotlin/app/readylytics/health/core/database/domain/scoring/RecomputeTodayUseCase.kt`                              | Scoring — settings recompute implementation         | executes under `HealthMutationCoordinator.withMutation`; defers via worker during maintenance |
...
| `feature/dashboard/src/main/kotlin/app/readylytics/health/feature/dashboard/InsightCard.kt`                | UI — component                                      | dismissible M3 health insight card + slim rerun restore state                            |
| `core/ui/src/main/kotlin/app/readylytics/health/core/ui/components/metriccard/UniversalMetricRenderers.kt` | UI — visualization                                  | soft arc gauge metric card renderer + delta pill                                         |

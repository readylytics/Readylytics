...
| `ComputeDailyTrimpUseCase`         | `core/scoring/src/main/kotlin/app/readylytics/health/domain/scoring/ComputeDailyTrimpUseCase.kt`         | Aggregates workout TRIMP values into daily raw TRIMP and derives model TRIMP updates for workouts.                                                                                                                                            |
...
| :--------------------------- | :-------------------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------- |
| `SleepScoringStrategy`       | `core/scoring/src/main/kotlin/app/readylytics/health/domain/scoring/strategies/SleepScoringStrategy.kt` | Sleep score = **Duration 40% + Architecture 20% + Restoration 25% + Fragmentation 15%** (default Balanced profile; selectable profiles). Continuous logistic curves for duration and efficiency, configurable oversleep dead zone (`hypersomniaOnsetRatio`), and schedule regularity penalty-only multiplier (0.92–1.00). |
| `SleepFragmentationCalculator` | `core/scoring/src/main/kotlin/app/readylytics/health/domain/scoring/sleep/SleepFragmentationCalculator.kt` | Pure Kotlin. Calculates Wake After Sleep Onset (`wasoMinutes`, wakefulness strictly between sleep onset and final awakening) and discrete awakening count (awake segments ≥90s) with adult grace allowance (20 min WASO / 2 awakenings). |
| `SleepModifierResolver`      | `core/scoring/src/main/kotlin/app/readylytics/health/domain/scoring/sleep/SleepModifierResolver.kt` | Assembles `SleepModifiers` (`fragmentation` and `regularityScore`). Fetches stages via `SleepSessionRepository.getSessionStages(sessionId)` (`SleepStageDao.getStagesForSession`) and live regularity via `CircadianConsistencyRepository.scoreFor(targetDate)`. Suppresses fragmentation when stages are missing or suspicious while retaining regularity. |
...
| `AssembleDailySummaryUseCase` | `core/scoring/src/main/kotlin/app/readylytics/health/domain/scoring/AssembleDailySummaryUseCase.kt` | Pure summary assembler for uncalibrated and calibrated pipeline branches in `ScoringRepositoryImpl`. |
...
**Settings gating signal.** The Settings screen no longer derives "is a resync running" from
`HistoricalResyncControllerImpl.state` (`WorkManager.getWorkInfosForUniqueWorkFlow(RESYNC_WORK_NAME)
.firstOrNull()`), which read the wrong entry in a multi-entry `APPEND_OR_REPLACE` chain and left the
Resync/Apply buttons enabled mid-resync. Instead it gates on `ForegroundSyncController.isResyncing` —
...
| `core/database/src/main/kotlin/app/readylytics/health/data/local/HealthDatabase.kt`                                             | Storage — Room DB (v12)                             | 17 entities; pre-bridge Room migration chain ends at v6; external migration owns v7; Room owns v7→v12 |
...
| `core/database/src/main/kotlin/app/readylytics/health/data/repository/ScoringDayDataLoader.kt`                               | Processing — scoring data loader                    | encapsulates 10 DAOs, loads/persists scoring entities                                   |
| `core/database/src/main/kotlin/app/readylytics/health/data/repository/ReadinessSummaryCoordinator.kt`                         | Processing — readiness summary coordinator          | coordinates sleep aggregation & calibrated/uncalibrated summary assembly               |
...
| `core/scoring/src/main/kotlin/app/readylytics/health/domain/scoring/ComputeDailyTrimpUseCase.kt`                               | Processing — daily workout TRIMP aggregation        | aggregates workout TRIMP & model TRIMP updates                                           |
...
| `core/scoring/src/main/kotlin/app/readylytics/health/domain/scoring/ResolveDailyBaselinesUseCase.kt`                           | Processing — baseline resolution                    | resolves pre- and post-computation baselines & frozen snapshots                          |
| `core/scoring/src/main/kotlin/app/readylytics/health/domain/scoring/AssembleDailySummaryUseCase.kt`                           | Processing — summary assembly                       | assembles uncalibrated & calibrated DailySummary snapshots                               |
...
| `core/scoring/src/main/kotlin/app/readylytics/health/domain/scoring/strategies/SleepScoringStrategy.kt`                        | Processing — sleep score                            | Duration 40% / Architecture 20% / Restoration 25% / Fragmentation 15% (default Balanced profile) |

...
| `PeriodicHealthSyncWorker`    | `workers/PeriodicHealthSyncWorker.kt`        | "Background Sync" toggle in Settings. `@HiltWorker` periodic **standard (non-foreground) worker** calling `HealthSyncUseCase.sync(windowDays = 2)` (shares `syncMutex` with the other two flows), bridges progress to `ForegroundSyncController`, shows/dismisses a silent transient notification (`SyncNotifications.BACKGROUND_SYNC_CHANNEL_ID`) via `NotificationManagerCompat` directly — no `setForeground()`, since `READ_HEALTH_DATA_IN_BACKGROUND` already permits background HC reads and starting a foreground service from a periodic background worker risks `ForegroundServiceStartNotAllowedException` on API 34+. `Result.retry()` on any sync failure. |
...
| `HealthChangeSynchronizer`    | `domain/sync/HealthChangeSynchronizer.kt`    | Reconciles differential Health Connect Changes API responses (upsertions and deletions) incrementally during daily/foreground sync. Resolves dates of deleted records via local DB lookup to properly invalidate and queue recompute of those dates. For composite-key metric tables (HR, HRV, weight, body-fat, blood pressure, SpO2), one HC source record can own multiple Room rows (`<sourceId>_<timestamp>`), so change handling now resolves and deletes **all** rows belonging to that source record before re-upserting the replacement. A changed record therefore recomputes from the earliest old/new affected date forward, and non-selected device updates delete the full local source-record footprint to avoid device contamination. |
...
| `ResyncCheckpointStore`       | `domain/sync/ResyncCheckpointStore.kt`       | Stores the resumable historical-resync checkpoint: fixed range, current phase (`INGEST` / `PRUNE` / `RECONCILE` / `RECOMPUTE`), next date, and selection hash. Cleared only after recompute and token refresh complete. |
...
> refreshes and persists next tokens at the end of its run, then clears the persisted resync
> checkpoint; if the worker dies earlier, the next retry resumes from the saved phase/date.
...
| `proto/resync_checkpoint.proto`                                            | Ingestion — checkpoint schema                       | Proto schema for historical resync phase/date checkpoint                               |

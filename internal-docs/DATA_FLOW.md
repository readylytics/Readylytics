...
| `core/model/src/main/kotlin/app/readylytics/health/domain/service/HealthMetricsService.kt`     | Domain — canonical BP classifier and facade          | `assessBmi()`/`assessBodyFatPercent()` → `BodyCompositionAssessment`; `assessBloodPressure()` classifies component-wise: ≤120/80 Optimal, otherwise ≤129/89 Neutral, otherwise ≤139/99 Warning, all others Poor |

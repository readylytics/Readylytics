...
| `HistoricalRecomputePhase`    | `core/healthconnect/src/main/kotlin/app/readylytics/health/core/healthconnect/domain/sync/HistoricalRecomputePhase.kt`    | Transactional walk-forward recompute loop extracted from `ResyncRangeUseCase` (WP-10). Executes daily recompute across the historical range in 30-day units (`RECOMPUTE_CHECKPOINT_INTERVAL_DAYS`), each unit running inside a single Room transaction via `DailyRecomputeSupport.inRecomputeTransaction { }`, checkpointed only after commit so transaction-rollback and checkpoint-resume boundaries coincide. Retains the immutable `runIdentity` across checkpoints. |
...
| `core/healthconnect/src/main/kotlin/app/readylytics/health/core/healthconnect/domain/sync/HistoricalRecomputePhase.kt`    | Ingestion — walk-forward recompute phase            | 30-day unit Room transactional walk-forward recompute and checkpoint commits            |

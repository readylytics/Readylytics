...
| `HealthChangeSynchronizer`    | `domain/sync/HealthChangeSynchronizer.kt`    | Reconciles differential Health Connect Changes API responses (upsertions and deletions) incrementally during daily/foreground sync. Resolves dates of deleted records via local DB lookup. Composite-key metric changes delete every Room row owned by the HC source record before re-upsert. The synchronizer returns candidate next tokens but never persists them; `HealthSyncUseCase` commits them only after requested-window ingest, reconciliation, step fetch, and scoring all succeed. Changes older than the requested scoring window leave tokens uncommitted and return `REQUIRES_HISTORICAL_RESYNC`, routing correction through the durable worker without widening foreground scoring. |

...
- `DatabaseMigrationModelsTest` — subject (`DatabaseReadinessGate`) now lives in `core:database`, but the test also asserts `V7_DATABASE_IDENTITY_HASH` (an `internal` const in the app's `V7DatabaseMigrator.kt`), so it cannot move. It stays in `:app` as an exception; the moved `DatabaseReadinessGate` internals it relies on (`ExistingDatabaseState`, the inject-less constructor, `CURRENT_DATABASE_VERSION`) were made non-`internal` in Task 10 to keep it compiling.

...
| Gate | Concrete proposed policy for review | Blocks |
|---|---|---|
| OD-1 | Preserve legacy warm rows as approximate/unknown. Complete authorized interval refresh may replace them. For newly rolled data, benchmark per-source, per-minute contributions before enabling that schema/publication; no exact deletion promise for lost legacy lineage. | T2/T3 schema and publication choice; corresponding archive fields |

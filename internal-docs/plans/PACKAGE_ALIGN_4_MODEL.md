...
- [ ] Task 1: procedure with `OLD=domain.logcat`, `NEW_SUBPATH=domain/logcat` (1 file).
- [ ] Task 2: `OLD=domain.error`, `NEW_SUBPATH=domain/error` (2).
- [ ] Task 3: `OLD=domain.display`, `NEW_SUBPATH=domain/display` (2).
- [ ] Task 4: `OLD=domain.cache`, `NEW_SUBPATH=domain/cache` (2).
- [ ] Task 5: `OLD=domain.circadian`, `NEW_SUBPATH=domain/circadian` (3).
- [ ] Task 6: `OLD=domain.audit`, `NEW_SUBPATH=domain/audit` (3).
- [ ] Task 7: `OLD=domain.githubissue`, `NEW_SUBPATH=domain/githubissue` (3).
- [ ] Task 8: `OLD=domain.crashreport`, `NEW_SUBPATH=domain/crashreport` (3).
- [ ] Task 9: `OLD=domain.vitals`, `NEW_SUBPATH=domain/vitals` (3, includes `@Serializable` `VitalsChartConfiguration.kt`).
- [ ] Task 10: `OLD=domain.date`, `NEW_SUBPATH=domain/date` (4).
- [ ] Task 11: `OLD=domain.heartrate`, `NEW_SUBPATH=domain/heartrate` (4 — includes `ZoneThresholds.kt`, referenced by DATA_FLOW.md).

## Tasks 12–18: medium packages (5–14 files)

- [ ] Task 12: `OLD=domain.preferences`, `NEW_SUBPATH=domain/preferences` (5; ~156 external importers — Step 4's grep enumerates them).
- [ ] Task 13: `OLD=domain.backup`, `NEW_SUBPATH=domain/backup` (6).
- [ ] Task 14: `OLD=domain.layout`, `NEW_SUBPATH=domain/layout` (6, `@Serializable` configs).
- [ ] Task 15: `OLD=domain.workouts`, `NEW_SUBPATH=domain/workouts` (6 — see the workouts.detail ordering note in the inventory; move only the files directly in `domain/workouts/`).
- [ ] Task 16: `OLD=domain.workouts.detail`, `NEW_SUBPATH=domain/workouts/detail` (7, `@Serializable` configs).
- [ ] Task 17: `OLD=domain.sleep`, `NEW_SUBPATH=domain/sleep` (12, several `@Serializable` configs + the sleep-profile weight models — diffs rename-only).
- [ ] Task 18: `OLD=domain.service`, `NEW_SUBPATH=domain/service` (14 — `HealthMetricsService.kt`, `BmiService.kt`, calculators; diffs rename-only).

## Task 19: `domain.repository` → `core.model.domain.repository` (16 files)

- [ ] Procedure with `OLD=domain.repository`, `NEW_SUBPATH=domain/repository` (~145 external importers — includes DATA_FLOW.md-referenced `HealthConnectRepository.kt`, `ScoringRepository.kt`, `ScoringHistoryRepository.kt`).

## Task 20: `domain.validation` → `core.model.domain.validation` (27 files)

- [ ] Procedure with `OLD=domain.validation`, `NEW_SUBPATH=domain/validation` (~15 external importers).
- [ ] **Extra check:** validation logic is imported project-wide; every hunk in `git diff --cached -M` under this package must be rename/package/import only. Any change to a validator's condition or message: revert.

## Task 21: `domain.model` → `core.model.domain.model` (47 files, widest blast radius)

- [ ] Procedure with `OLD=domain.model`, `NEW_SUBPATH=domain/model`.
- [ ] **Extra pre-check before Step 4:** count importers (`grep -rl "^import app\.readylytics\.health\.domain\.model\." app core feature database-benchmark --include='*.kt' | grep -v '/build/' | wc -l`) and record the number in the commit message body (~313 expected). After the rename, re-running the same grep with the new prefix must return the same count.
- [ ] Step 5 has the known inline `app.readylytics.health.domain.model.Result` usages in worker tests (PeriodicHealthSyncWorkerTest, HealthResyncWorkerTest) — rewrite each.
- [ ] This package holds DATA_FLOW.md-referenced files (`BodyCompositionAssessment.kt`, `DailyMetricsMapper.kt`, `HealthConnectRecords.kt`, `InsightType.kt`, `LoadSourceSelector.kt`, `VitalStatusClassifiers.kt`) — Step 8 covers all.
- [ ] Run the full gate before committing, not just `:core:model` tests — this rename touches every module.
...
- [ ] Move the 3 files (`HeartRateMapperTest.kt`, `StepsMapperTest.kt`, `WorkoutMapperTest.kt`) into `core/model/src/test/kotlin/app/readylytics/health/core/model/domain/sync/mappers/`.
- [ ] Rewrite their package lines to `package app.readylytics.health.core.model.domain.sync.mappers` and drop now-redundant imports of that package (Step 6 compile confirms).
- [ ] Steps 6–14 of the standard procedure (skipping Step 4's repo-wide import rewrite — nothing imports these tests; verify with the sweep grep).
- [ ] Commit: `refactor: align core/model data.healthconnect test package with module namespace`

## Task 23: DATA_FLOW.md sweep + architecture-test namespace narrowing

- [ ] `grep -n 'core/model/src' internal-docs/DATA_FLOW.md` — every path must now point at an on-disk file. Fix remaining stale segments (including `health/domain/heartrate/`, `health/domain/migration/`, `health/domain/model/`, `health/domain/repository/`, `health/domain/service/`, `health/domain/sleep/`, `health/domain/sync/...`, `health/domain/util/`, `health/domain/workouts/`, `health/data/preferences/` references — most covered by per-task Step 8, this is the backstop).
- [ ] Spot-check 5 resolved paths with `test -f`.
- [ ] **Namespace narrowing (per the follow-ups doc's "When all renames are done"):** once Plans 1–4 are all complete and `grep -rl '^package app\.readylytics\.health\.\(domain\|data\)\.' core feature --include='*.kt'` is empty, narrow the legacy entries in `app/src/test/.../CleanArchTest.kt` — `"app.readylytics.health.domain.."` (:11) and `"app.readylytics.health.data."` (:21) in `domainPackageGlobs`/`dataLayerPackagePrefixes` — to cover only the `app` module's own domain/data packages (e.g. `"app.readylytics.health.domain.migration.."`, and the specific `app/.../data/**` roots that remain in `app/src/main`). Enumerate what actually remains under `app/src/main/kotlin/app/readylytics/health/domain/` and `.../data/` first, and derive the narrowed globs from that measurement. Do NOT delete the legacy entries while any flat-ns files remain in `app`'s own tree.
- [ ] `./gradlew :app:testDebugUnitTest` — `CleanArchTest` and `FeatureModuleArchitectureTest` must pass with the narrowed lists.
- [ ] Commit: `docs: narrow legacy namespace globs in CleanArchTest after package alignment` (split into two commits if the DATA_FLOW backstop found changes of its own).

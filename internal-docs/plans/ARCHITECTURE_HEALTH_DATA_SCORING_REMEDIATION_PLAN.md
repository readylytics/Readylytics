...
**Decision (2026-08-31): benchmark execution skipped.** Steps 4–6 (the `:database-benchmark`
microbenchmarks B1–B7, the `QueryPlanTest` B9, and the `:benchmark` startup trace B10) were
**not executed**. The `:database-benchmark` module was scaffolded and the benchmark classes were
written and compiled, but every `connectedBenchmarkAndroidTest` run against the local physical
device failed or stalled: R8 stripped classes the `androidx.test` runner needs from the minified
benchmark variant (fixable only by disabling minification — a config change the benchmark numbers
would then not represent), the `IsolationActivity`/resource wiring is fragile, and the
100 k-row workloads exceeded practical run budgets. This matches the documented limitation in
`POST_REMEDIATION_FOLLOWUPS` item 3 ("the instrumented suite is known not to be reliably runnable
on a local physical device"). The benchmark scaffolding was **reverted** to keep Phase 0 free of
unrunnable code. **No Phase-0 baseline numbers exist in §11.** Phase 2's benchmarks (B2, B3),
Phase 4's (B1, B4, B6) and the B9/B10 assertions therefore have no Phase-0 baseline to compare
against — they must be run on a working instrumented environment (managed device/emulator or a
device the suite runs on) before Phase 2, and the measurements recorded then become the baseline.
The `R2-DI-002`, `R2-DB-001` and `R2-DB-004` work below is complete and unblocked.
...
`app/src/profileSupport/.../BenchmarkDataSeeder.kt`. *(Benchmark files were reverted per the
decision above.)*
...
**Required validation.** Full pre-commit chain. Benchmarks were skipped by decision
(see the **Decision** block above) — their numbers are not recorded.

**Completion criteria.**
- [x] `grep -rn "Instant.now()\|System.currentTimeMillis()" core/healthconnect/src/main core/model/src/main/kotlin/app/readylytics/health/core/model/domain/util/RetentionBounds.kt app/src/main/kotlin/app/readylytics/health/workers` returns nothing.
- [x] Characterization tests exist, pass, and are annotated with the work package that flips them.
- [ ] All four benchmarks produce recorded baseline numbers in §11. — **SKIPPED (decision 2026-08-31)**; baselines must be captured on a working instrumented environment before Phase 2.
- [ ] `R2-PERF-005` is resolved to confirmed-or-closed with a trace as evidence. — **SKIPPED (decision 2026-08-31)**; remains unresolved/suspected.
...
|---|---|---|---|---|---|
| B1 | `HcReadTransformBenchmark` | wall time + allocations for mapping one 5,000-sample HC page (`HeartRateMapper.mapToInputs`) | skipped — see §9 | ≥ 50 % fewer allocated objects; output element-identical | `R2-PERF-004` |
| B2 | `HrUpsertBenchmark` | throughput of `persistHeartRateSamples` for 100 k rows, cold and idempotent-re-ingest | skipped — see §9 | measurable improvement **or** finding closed with the measurement | `R2-PERF-003` |
| B3 | `RollupBenchmark` | wall time **and peak WAL** (`RoomWalDiagnostics.walFileSizeInfo()`) rolling up `F-DENSE-30D` | skipped — see §9 | WAL stays within 2× of one day's rollup; transaction count ≥ days rolled | `R2-PERF-002` |
| B4 | `WarmReconstructionBenchmark` | allocated bytes/objects building a 30-day warm baseline window from `F-WARM` | skipped — see §9 | ≥ 80 % reduction; zero boxed objects | `R2-PERF-001` |
| B5 | `HeartRateAggregationBenchmark` | `getMinuteBuckets` (hot), `getMinuteBuckets` (warm), `loadMergedMinuteBuckets` over one day of `F-DENSE-30D` | skipped — see §9 | no regression | — |
| B6 | `IncrementalRecomputeBenchmark` | `recomputeRange` over 8 days (the daily-sync widened window) on `F-MULTIYEAR` | skipped — see §9 | no regression; used to resolve OD-5 | `R2-CACHE-002` |
| B7 | `HistoricalRebuildBenchmark` | `resyncRange(skipIngestAndPrune = true)` over 365 days of `F-MULTIYEAR` | skipped — see §9 | peak heap does not scale with total history length | `R2-PERF-001`, `R2-DB-001` |
...
| B10 | `:benchmark` startup macrobenchmark | cold-start trace; presence of `migrateIfNeeded`/Keystore work on the main thread | skipped — see §9 | resolves `R2-PERF-005` either way | `R2-PERF-005` |
...
*Status (2026-08-31).* **Unresolved.** Phase-0 benchmarks were skipped by decision (see §9 Phase 0) —
the startup trace was never captured, so `R2-PERF-005` remains **suspected** and Phase-4 step 7
must not be actioned until a trace is captured on a working instrumented environment.

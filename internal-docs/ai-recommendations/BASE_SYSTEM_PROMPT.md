...
receive. Do not interpret the Strain Ratio numeric value directly. Use `recommended_load`
(Section 3) as the authoritative ceiling for how much to recommend, and `load_context`
only to describe the current load state qualitatively (e.g. "your training load is in
the sweet spot") rather than saying "Strain Ratio" or a raw ratio value.
...
- **HRV/RHR baselines & z-scores** — each person's HRV baseline is a rolling,
  ...
  `zRhr` are classic under-recovery signals. They help you understand *why* a derived
  value (e.g. a lower Restoration/Sleep Score or a POOR/WARNING readiness band) looks
  the way it does — never as an independent score. User-facing `rationale` should
  primarily cite named derived values (bands, load context, flags), not raw HRV/RHR
  numbers; mention a raw z-score only if no derived value captures it and it is
  decision-relevant.
...
- `recommended_load` is ALREADY adjusted upstream for today's completed workouts. It
  represents the *remaining* envelope. Never subtract, reduce, or downgrade it using
  `today_trimp` or `today_training_minutes` — that double-counts training the
  upstream calculation has already accounted for.

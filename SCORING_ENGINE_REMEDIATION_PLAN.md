...
> (Audit row, Open Risk, or Clarification Q) it resolves. Nothing here is invented.
...
dead code, and documentation* around that validated math.
...
| Profiles | Shift Worker documented but unimplemented (Audit MISMATCH, Q2) | Remove Shift Worker; migrate existing users → Active |
...
- Sleep Score weighting, lnRMSSD methodology, RHR percentile methodology, ATL/CTL EMA,
  Strain Ratio + Load Gaussian decay, TRIMP (Banister/Cheng/iTRIMP), PAI, recovery caps,
  σ-blend baselines.
...
| M3 | **Shift Worker descope.** Documented per-weekday logic does not exist; strategy returns `Int.MAX_VALUE`. | Audit MISMATCH; Q2 | Product decision #3: remove Shift Worker, migrate → Active. |
...
| L2 | **Two HRV-baseline statistics coexist.** `computeHrvBaseline` returns arithmetic median of nightly means; frozen z-score baseline is geometric `exp(mean(ln))`. | Risk 6; Q7 | Display-only ambiguity; no math change. Carried to Remaining Product Questions. |
| L3 | **Stale DATA_FLOW package root.** Document roots paths at `com/gregor/lauritz/healthdashboard/`; code lives at `app/readylytics/health/`. | Audit MISMATCH; Risk 3 | Pure documentation defect; every path citation is wrong. |
| L4 | **In-app About strings drifted from BOTH ABOUT.md and code.** e.g. flat ±30 threshold copy, Load sweet-spot "0.8–1.3", decay "above 1.5", day-range phases. | New finding (content audit); supports Q1/Q3/Q4/Q6 | The in-app About page is a third, independently-wrong narrative. Must be reconciled and guarded. |
...
- **Keep** `CircadianThresholdDefaults` (`domain/circadian/CircadianThresholdDefaults.kt`)
  as the *only* threshold map: `ATHLETE → 20`, `ACTIVE → 30`, `GENERAL → 30`,
  `SEDENTARY → 45`. Remove the `SHIFT_WORKER` entry (see §4).
...
- This change alters displayed circadian scores for Athlete/Sedentary users who relied on
  ...
  profile thresholds). Document it in release notes (see §7/§11).
...
## 4. Shift Worker Removal Plan

**Goal (product-fixed):** remove the Shift Worker profile; migrate existing users to
**Active**. (Resolves M3; Audit MISMATCH; Q2.)

### 4.1 This is a product decision, not a neutral migration

Today `SHIFT_WORKER`'s physiology constants are **identical to `SEDENTARY`**
(`PhysiologyProfile.kt:38-46`: `lnSigmaPrior 0.20`, `banisterMultiplier 1.75`,
...
(tighter HRV/PAI tuning: `lnSigmaPrior 0.15`, `banisterMultiplier 1.35`,
`PAI_SCALING_ACTIVE` vs `_SHIFT_WORKER 0.20`, HRV sensitivity ±1.5 vs ±2.0). The plan must
state this trade-off plainly and surface an optional **one-time in-app notice** so migrated
users learn their profile changed and can re-select Sedentary if they prefer.

### 4.2 Inventory of references (~31 sites)

| Category | Locations |
| :-- | :-- |
| Enum definition | `data/preferences/PhysiologyProfile.kt:38-46` (`SHIFT_WORKER`) |
| Proto / serialization | `proto/user_preferences.proto` (`PhysiologyProfileProto.PROFILE_SHIFT_WORKER = 4`); `data/preferences/PhysiologyPreferences.kt:160`; `UserPreferencesSerializer.kt:148`; `UserPreferences.kt:89`; `di/DataStoreModule.kt:233` |
| Backup / restore | `data/backup/LocalRestoreManager.kt:459` (`PhysiologyProfileProto.valueOf(it)`) |
| Scoring config | `ScoringConfigFactory.kt:88,103,116,130`; `PaiCalculator.kt:14`; `ScoringConstants.kt` (`PAI_SCALING_SHIFT_WORKER = 0.20`) |
| Circadian | `domain/circadian/CircadianStrategyFactory.kt:8`; `CircadianThresholdDefaults.kt:12-14`; `RegularUserCircadianStrategy.kt:15`; `ShiftWorkerCircadianStrategy.kt` (whole file) |
| UI | `ui/components/PhysiologyProfilePicker.kt:20`; `ui/settings/CircadianThresholdSettingsSection.kt:38,98-100,151,351-355`; `ui/settings/SettingsScreen.kt:455` |
| Strings | `res/values/strings.xml` (`profile_title_shift_worker`; `about_circadian_caveat` shift mention) |
| Tests | `domain/circadian/CircadianStrategyTest.kt:9,43-54`; `data/local/CircadianStrategyTest.kt:9`; `domain/scoring/PaiCalculatorTest.kt:21` |

### 4.3 Removal & migration strategy

1. **Domain enum:** delete `SHIFT_WORKER` from `PhysiologyProfile`. Remaining: `ATHLETE,
   ACTIVE, GENERAL, SEDENTARY` (the product's "Athlete/Active/Sedentary" plus the existing
   internal `GENERAL` default — confirm whether `GENERAL` is surfaced to users or is an
   internal default; if user-visible it maps to "Active" in copy. **Open: confirm GENERAL's
   user-facing identity** — see Remaining Product Questions).
2. **Proto:** **reserve** `PROFILE_SHIFT_WORKER = 4`; never delete or reuse the ordinal
   (proto compatibility). Add `reserved 4;` / `reserved "PROFILE_SHIFT_WORKER";` after
   removing the enumerator, OR keep the enumerator and only remove it from the domain — the
   safer path is to **keep the proto value** and map it at the boundary.
3. **Read boundary mapping (no data loss):** wherever proto → domain conversion happens
   (`UserPreferences.kt:89`, `DataStoreModule.kt:233`, `LocalRestoreManager.kt:459`),
   translate `PROFILE_SHIFT_WORKER` → `ACTIVE`. This makes any stored or restored
   Shift-Worker value resolve to Active even before the rewrite migration runs.
4. **One-time DataStore migration:** rewrite persisted `PROFILE_SHIFT_WORKER` →
   `PROFILE_ACTIVE` so the stored value is canonical going forward.
5. **Scoring config / circadian / UI / strings / tests:** delete all Shift Worker branches,
   the `ShiftWorkerCircadianStrategy`, `PAI_SCALING_SHIFT_WORKER`, the picker entry, the
   settings `ShiftWorkerModeSelector`, `profile_title_shift_worker`, and the shift mention in
   `about_circadian_caveat`. Update/remove the named tests; add a migration test (below).
6. **Profile snapshot history:** `DailySummaryEntity.snapshotProfile` may contain
   `SHIFT_WORKER` on historical rows. Apply the same read-time mapping → `ACTIVE` (or leave
   historical snapshots intact as an immutable record and only map on read for display).
   Document the choice.

### 4.4 Rollout

Ship behind the normal release; the read-boundary mapping (step 3) guarantees safety even
if the rewrite migration (step 4) is deferred or fails. Validate with the migration test
and a backup-restore round-trip test before release. Include the one-time in-app notice.
...
## 6. Documentation Synchronization Architecture
...
### 6.1 The drift problem (confirmed)
...
detector in §10). Pair it with the governance rules in §9 so prose updates travel together
...
| Load curve (Q4/L4) | `sr ≤ 1.3 → 100`; `sr > 1.3 → 100·exp(−2.5·(sr−1.3)²)` (Gaussian decay from 1.3, **no** 0.8 floor, **no** 1.3–1.5 linear segment). | `about_ratio_sweet_spot` ("0.8–1.3"), `about_ratio_quadratic` ("above 1.5") |
| Profiles | **Athlete / Active / Sedentary** only. Remove all Shift Worker copy. | ABOUT.md profile table & "Special handling for shift workers"; `about_circadian_caveat`; `profile_title_shift_worker` |
...
| Package root (L3) | `app/readylytics/health/` (not `com/gregor/lauritz/healthdashboard/`). | `DATA_FLOW.md:14` and every path citation |

### Per-artifact plan

- **`ABOUT.md`:** apply all corrections above; rewrite the "Your profile matters" and
  "Circadian Consistency" sections (drop Shift Worker); replace the "How long until your
  scores stabilise" section with the session-count 4-phase model; add a "Hidden modifiers"
  / "Score adjustments" subsection; add a short "Determinism & timezone" note; reconcile the
  Load §4 "~1.5" narrative with the 1.3 decay onset (Q4).
- **`docs/DATA_FLOW.md`:** fix the package root (L3); correct the §2.5 caps wording (M5);
  add ownership pointers for the new `scoringZoneId`, the resolved circadian threshold, and
  the session-count phase calculator; note Shift Worker removal.
- **In-app About page (`strings.xml` `about_*`):** apply every content correction;
  ...
  `about_circadian_caveat` shift mention; add modifier/timezone strings.
- **Score explanation screens / tooltips (`tooltip_*`, `MetricTooltip.kt` consumers):**
  align readiness, circadian, load, and phase tooltips with the corrected numbers.
...
**Goal:** no documentation drift — the same numbers and phase/profile/threshold definitions
appear identically across all surfaces.

---

## 8. In-App About Page Remediation
...
| :-- | :-- | :-- | :-- | :-- |
| About page (scoring sections) | `strings.xml` `about_*` (~76 keys) → `AboutScreen.kt` / `AboutComponents.kt` | About page | Apply §7 corrections; remove Shift Worker; rewrite threshold/ratio/phase keys | Drift test (§10) asserts numbers vs ABOUT.md + constants; Compose render test that key sections show |
| Score explanation screens | `strings.xml` (per-metric) | Dashboard/detail VMs | Align readiness/circadian/load wording | Drift test + screenshot/semantics test |
| Tooltips | `tooltip_*` via `MetricTooltip` | Metric cards | Correct numbers (thresholds, sweet spot, caps) | Drift test covers tooltip numeric claims |
| Help dialogs / educational cards | `strings.xml` | Respective screens | Same corrections | Drift test |
| Onboarding explanations | `onboarding_feature_*` + profile picker | `ui/onboarding/**` | Athlete/Active/Sedentary only; profile-sets-threshold note | Picker test asserts 3 profiles, no Shift Worker |
...
- Shift Worker is absent from every user-facing surface.
...
## 9. CLAUDE.md Governance Update Plan
...
### 9.1 Documentation Synchronization Rule (new)

> Any PR that changes formulas, thresholds, coefficients, scoring behavior, readiness
> logic, sleep logic, circadian logic, recovery logic, the phase model, or onboarding
> scoring explanations **must**, in the same PR, update: `ABOUT.md`, the relevant
> `docs/DATA_FLOW.md` sections, the in-app About page (`about_*` / `tooltip_*` strings), and
> all affected user-facing scoring explanations. Treat a stale scoring doc as a broken build.

### 9.2 Documentation Review Checklist (new)
...
### 9.3 Single-source-of-truth strategy (new)

State that `ABOUT.md` is the canonical scoring narrative and the in-app `about_*` strings
are the i18n delivery copy, kept consistent by the automated **drift-detection test** (§10).
Record the options/pros-cons from §6 and that Option A is adopted. Extend the existing
"`DATA_FLOW.md` is load-bearing" clause to explicitly cover `ABOUT.md` and the `about_*`
strings.

### 9.4 Recommended single-source approach (answer to the task's question)
...
### Migration (Shift Worker)
- DataStore migration: stored `PROFILE_SHIFT_WORKER` → `PROFILE_ACTIVE`.
- Read-boundary mapping: proto `PROFILE_SHIFT_WORKER` resolves to `ACTIVE` before rewrite.
- Backup/restore round-trip: a backup containing Shift Worker restores as Active.
- Historical `snapshotProfile = SHIFT_WORKER` rows resolve/display as Active per chosen policy.
...
- A presence test: every profile/threshold/phase value referenced in `ABOUT.md` exists in
  `strings.xml`, and no surface mentions "Shift Worker".
...
| Shift Worker removal (§4) | Low | **Medium** — proto/enum/backups | Medium — affected users change profile | Low — proto value reserved, read-mapped, then rewritten | Reserve ordinal 4; read-boundary map → Active; rewrite migration; backup round-trip test; in-app notice |
...
| Documentation + drift tests (§6–§9) | Low | None | Positive — accurate docs | None | Drift test guards numbers; review checklist guards prose |
...
| **3. Shift Worker removal** | Enum/proto/config/UI/strings/tests; read-map + rewrite migration; in-app notice | Depends on circadian (shares profile/threshold code) | Migration + backup round-trip tests green |
...
| **5. Documentation + governance + drift tests** | Apply §7/§8 corrections across all surfaces; CLAUDE.md/AGENTS.md rules; drift-detection + presence tests | Locks in the now-final constants/strings | Drift test green; no surface mentions Shift Worker; all numbers reconciled |

Behavior-independent doc fixes (e.g. the DATA_FLOW package-root path, L3) may ride along in
any earlier phase, but the automated guardrail is established in Phase 5.

---

## Remaining Product Questions

Only genuinely unresolved items remain (the six product decisions settled the rest):

1. **Q7 — HRV baseline statistic for display.** `computeHrvBaseline` returns the arithmetic
   median of nightly means; the frozen z-score baseline is the geometric `exp(mean(ln))`.
   Which should charts/cards display? Display-only; **no math change** either way.
2. **`GENERAL` profile's user-facing identity.** The domain enum has `ATHLETE, ACTIVE,
   GENERAL, SEDENTARY`; product copy says "Athlete / Active / Sedentary." Confirm whether
   `GENERAL` is an internal default that should be presented to users as "Active" (its
   constants differ slightly: `lnSigmaPrior 0.18` vs Active `0.15`) or kept distinct. This
   affects onboarding/About copy and the Shift Worker→Active mapping label.

*Resolved during planning (recorded for traceability, not open):* Shift Worker → **Active**
is a confirmed deliberate product decision even though today's Shift constants match
**Sedentary** (documented as a behavior change in §4); the documentation single-source
approach is **canonical ABOUT.md + drift tests** (§6).

---

*This plan is implementation-ready: every recommendation traces to a confirmed finding in
`SCORING_ENGINE_DISCOVERY_REPORT.md`, and no change alters the scientifically validated
scoring mathematics.*

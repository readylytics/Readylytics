...
object DashboardMetricScalePreparer {
    ...
    private fun linearFraction(value: Float, minimum: Float, maximum: Float): Float {
    ...
    fun goal(
        ...
        val markerFraction = if (isValidTarget && value != null) {
            linearFraction(value, 0f, target!!)
        } else null
        
        val mappedBands = if (isValidTarget) {
            bands.map {
                DashboardMetricBand(
    ...
    fun personalBaseline(
        ...
        val markerFraction = if (isValidBaseline && value != null) {
            linearFraction(value, minVal, maxVal)
        } else null
        
        val baselineMarkerFraction = if (isValidBaseline) {
            linearFraction(baseline!!, minVal, maxVal)
        } else 0f
        
        val mappedBands = if (isValidBaseline) {
            bands.map {
                DashboardMetricBand(
    ...
    fun referenceRange(
        ...
        val markerFraction = if (scaleAvailable && value != null) {
            piecewiseFraction(value, minimum, midpoint, maximum)
        } else null
        
        val referenceMarkerFraction = if (scaleAvailable) {
            piecewiseFraction(midpoint, minimum, midpoint, maximum)
        } else null
        
        val mappedBands = if (scaleAvailable) {
            bands.map {
                DashboardMetricBand(
        ...
        val finalUnavailableReason = if (scaleAvailable && value == null) {

...
class DashboardCardioMetricPresentationFactoryTest {
    ...
    fun setUp() {
        ...
        every { resourceProvider.getString(CoreUiR.string.tooltip_cardio_fitness) } returns "tooltip cardio fitness"
    ...
    @Test
    fun cardioFitness_withData_hasUnitAndTooltip() {
        val summary =
            DailySummary(
        ...
        assertEquals("48", cardio?.valueText)
        assertEquals("ml/kg/min", cardio?.unitText)
        assertEquals("tooltip cardio fitness", cardio?.tooltip)
        assertTrue(cardio?.secondaryText?.contains("Wearable") == true)
        assertEquals(MetricStatus.OPTIMAL, cardio?.status)
    }

    @Test
    fun cardioFitness_noData_hasUnitTooltipAndNoDataStatus() {

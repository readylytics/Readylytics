...
class RetentionCleanupTest {
    ...
    @Test
    fun testRetentionCleanup() = runTest {
        ...
        sleepStageDao.upsertAll(
            listOf(
                SleepStageEntity(id = 1, sessionId = "old_sleep", stageType = "REM", startTime = cutoffMs - 2000, endTime = cutoffMs - 1500, durationMinutes = 10),
                SleepStageEntity(id = 2, sessionId = "equal_sleep", stageType = "DEEP", startTime = cutoffMs - 1000, endTime = cutoffMs - 500, durationMinutes = 10),
                SleepStageEntity(id = 3, sessionId = "new_sleep", stageType = "LIGHT", startTime = cutoffMs, endTime = cutoffMs + 500, durationMinutes = 10)
            )
        )
        ...
        heartRateDao.upsertAll(
            listOf(
                HeartRateRecordEntity(id = "old_hr", timestampMs = cutoffMs - 1, beatsPerMinute = 70, recordType = "RESTING"),
                HeartRateRecordEntity(id = "equal_hr", timestampMs = cutoffMs, beatsPerMinute = 70, recordType = "RESTING"),
                HeartRateRecordEntity(id = "new_hr", timestampMs = cutoffMs + 1, beatsPerMinute = 70, recordType = "RESTING")
            )
        )
        ...
        bloodPressureDao.upsertAll(
            listOf(
                BloodPressureRecordEntity(id = "old_bp", timestampMs = cutoffMs - 1, systolicMmHg = 120, diastolicMmHg = 80),
                BloodPressureRecordEntity(id = "equal_bp", timestampMs = cutoffMs, systolicMmHg = 120, diastolicMmHg = 80),
                BloodPressureRecordEntity(id = "new_bp", timestampMs = cutoffMs + 1, systolicMmHg = 120, diastolicMmHg = 80)
            )
        )
        ...
        val stagesCount = database.openHelper.writableDatabase.compileStatement("SELECT COUNT(*) FROM sleep_stages").simpleQueryForLong()

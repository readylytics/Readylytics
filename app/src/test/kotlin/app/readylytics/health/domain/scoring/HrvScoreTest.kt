...
class HrvScoreTest {
    ...
    fun `z=-3 clamps to 0`() {
        ...
        assertEquals(0f, calculator.computeHrvScore(-3f), DELTA)
    }

    @Test
    fun `z=-2 gives 0`() {
        // 50 + 25 * (-2) = 0
        assertEquals(0f, calculator.computeHrvScore(-2f), DELTA)
    }

    @Test
    fun `z=-1 gives 25`() {
        assertEquals(25f, calculator.computeHrvScore(-1f), DELTA)
    }

    @Test
    fun `z=0 gives 50`() {
        assertEquals(50f, calculator.computeHrvScore(0f), DELTA)
    }

    @Test
    fun `z=1_5 gives 87_5`() {
        // 50 + 25 * 1.5 = 87.5
        assertEquals(87.5f, calculator.computeHrvScore(1.5f), DELTA)
    }
    ...
    fun `z=2_0 gives 90_625 (saturated)`() {
        ...
        // 50 + 25 * 1.625 = 90.625
        assertEquals(90.625f, calculator.computeHrvScore(2f), DELTA)
    }
    ...
    fun `z=3_0 gives 96_875 (saturated)`() {
        ...
        // 50 + 25 * 1.875 = 96.875
        assertEquals(96.875f, calculator.computeHrvScore(3f), DELTA)
    }

    @Test
    fun `z=3 is strictly less than 100 (soft saturation)`() {
        assertTrue("Score at z=3 should be < 100", calculator.computeHrvScore(3f) < 100f)
    }
    ...
    fun `result never exceeds 100`() {
        for (z in listOf(1.5f, 2f, 3f, 5f, 10f)) {
            assertTrue("Score at z=$z should be <= 100", calculator.computeHrvScore(z) <= 100f)
        }
    }
    ...
    fun `result never goes below 0`() {
        for (z in listOf(-1.5f, -2f, -3f, -5f, -10f)) {
            assertTrue("Score at z=$z should be >= 0", calculator.computeHrvScore(z) >= 0f)
        }
    }
}

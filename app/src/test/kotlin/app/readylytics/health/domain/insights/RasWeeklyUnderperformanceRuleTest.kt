...
class RasWeeklyUnderperformanceRuleTest {
    ...
    private fun context(
        ...
        today = dailySummary(date = today, legacyTotalRas = todayTotalPai),
    ...
    fun `fires when weekly RAS total is below target`() {
        val recent =
            (1..6).map { offset -> dailySummary(date = today.minusDays(offset.toLong()), legacyTotalRas = 10f) }
    ...
    fun `does not fire when weekly RAS meets target`() {
        val recent =
            (1..6).map { offset -> dailySummary(date = today.minusDays(offset.toLong()), legacyTotalRas = 25f) }
    ...
    fun `does not fire when weekly RAS equals target exactly`() {
        val recent =
            (1..6).map { offset -> dailySummary(date = today.minusDays(offset.toLong()), legacyTotalRas = 0f) }
    ...
    fun `treats null totalRas days as zero when at least one day has data`() {
        val recent =
            listOf(
                dailySummary(date = today.minusDays(1), legacyTotalRas = null),
                dailySummary(date = today.minusDays(2), legacyTotalRas = null),
            )
    ...
    fun `deduplicates days by date when recentDays includes today`() {
        val recent = listOf(dailySummary(date = today, legacyTotalRas = 10f))
        val finding = rule.evaluate(context(todayTotalPai = 10f, recentDays = recent))

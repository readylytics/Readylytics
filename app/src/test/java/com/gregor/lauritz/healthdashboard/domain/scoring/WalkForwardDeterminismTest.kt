...
class WalkForwardDeterminismTest {
    ...
    private val allSessions = (0 until totalDays).map { i ->
    ...
    fun setup() {
        ...
        coEvery {
            ...
            rmssdValid = true, rhrValid = true, durationValid = true,
            stagesValid = true, stagesSuspicious = false, hrCoverageValid = true,
        )
    ...
    @Test
    fun `baselines for day 60 are identical whether Room has 60 or 120 days`() = runTest {
        // Scenario A: Room has only 60 days (onboarding-equivalent).
        val sessionsA = allSessions.take(61) // days 0-60
        coEvery { sleepSessionDao.getBetween(any(), any()) } answers {
            val fromMs = firstArg<Long>()
            val toMs = secondArg<Long>()
            sessionsA.filter { it.startTime >= fromMs && it.endTime <= toMs }
                .sortedBy { it.startTime }
        }
        ...
        val baselinesA = baselineComputer.computeHrvWindowsBetween(
            fromMs = targetDayMs, toMs = targetDayEndMs, excludeSessionId = allSessions[60].id,
        )
        ...
        val baselinesB = baselineComputer.computeHrvWindowsBetween(
            fromMs = targetDayMs, toMs = targetDayEndMs, excludeSessionId = allSessions[60].id,
        )

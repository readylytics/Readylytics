...
class DashboardFlowIntermediateTest {
    ...
    @Test
    fun `createDashboardBasicInputsFlow emits basicInputs for current date`() = runTest {
        ...
        val basicInputsFlow = createDashboardBasicInputsFlow(
            selectedDateFlow,
    ...
    @Test
    fun `createDashboardBasicInputsFlow emits basicInputs for historical date`() = runTest {
    ...
    @Test
    fun `createDashboardBasicInputsFlow includes 7-day PAI breakdown`() = runTest {
        val today = LocalDate.now()
        val paiSummaries = listOf(
            DailySummary(
                dateMs = today.minusDays(6).atStartOfDay().toInstant().toEpochMilli(),
                stepCount = 1000,
                isCalibrating = false,
                dataQuality = 1.0f,
            ),
    ...
    @Test
    fun `createDashboardCardStateFlow emits cardState`() = runTest {
        val today = LocalDate.now()
        ...
        val selectedDateFlow = flowOf(today)
        val cardStateFlow = createDashboardCardStateFlow(
            selectedDateFlow,
    ...
    @Test
    fun `createDashboardCardStateFlow handles null sleep session`() = runTest {
        val today = LocalDate.now()
        val cardConfig = CardConfiguration(
            cardId = CardId.SLEEP,
            isVisible = true,
    ...
    @Test
    fun `createDashboardRealtimeStateFlow emits sync state`() = runTest {
        coEvery {
    ...
    @Test
    fun `combineDashboardInputs aggregates all three flows`() = runTest {
        ...
        val combinedFlow = combineDashboardInputs(
            basicInputsFlow,
            cardStateFlow,
            realtimeStateFlow,
        )

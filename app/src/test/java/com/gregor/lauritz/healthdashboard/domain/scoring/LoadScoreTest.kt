...
class LoadScoreTest {
    ...
    fun `SR 1_35 begins quadratic decay`() {
        ...
        assertEquals(99.4f, calculator.computeLoadScore(1.35f), DELTA)
    }

    @Test
    fun `SR 1_5 moderate overload`() {
        // 100 * exp(-2.5 * 0.2^2) = 100 * exp(-0.1) ≈ 90.5
        assertEquals(90.5f, calculator.computeLoadScore(1.5f), DELTA)
    }

    @Test
    fun `SR 2_0 heavy overload`() {
        // 100 * exp(-2.5 * 0.7^2) = 100 * exp(-1.225) ≈ 29.4
        assertEquals(29.4f, calculator.computeLoadScore(2.0f), DELTA)
    }

    @Test
    fun `SR 3_0 extreme overload approaches zero`() {
        // 100 * exp(-2.5 * 1.7^2) = 100 * exp(-7.225) < 1
        assert(calculator.computeLoadScore(3.0f) < 1f)
    }

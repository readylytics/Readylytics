...
class HrvSigmaTest {
    ...
    fun `empty list returns prior when n is 0`() {
        val sigma = ScoringCalculator.hrvSigma(emptyList(), sigmaPrior = PhysiologyProfile.GENERAL.lnSigmaPrior)
    ...
    fun `at n=7 w is zero, result is prior`() {
        ...
        val sigma = ScoringCalculator.hrvSigma(lnList, sigmaPrior = PhysiologyProfile.GENERAL.lnSigmaPrior)
        assertEquals(0.18f, sigma, DELTA)
    }

    @Test
    fun `athlete profile uses smaller prior than sedentary at n=0`() {
        val athlete  = ScoringCalculator.hrvSigma(emptyList(), sigmaPrior = PhysiologyProfile.ATHLETE.lnSigmaPrior)
        val sedentary = ScoringCalculator.hrvSigma(emptyList(), sigmaPrior = PhysiologyProfile.SEDENTARY.lnSigmaPrior)
    ...
    fun `sigma floor is always respected`() {
        for (n in 0..65) {
            ...
            val sigma = ScoringCalculator.hrvSigma(lnList)
    ...
    fun `at n=20 sigma is a blend between personal and prior`() {
        ...
        val sigma = ScoringCalculator.hrvSigma(lnList, sigmaPrior = 0.18f)

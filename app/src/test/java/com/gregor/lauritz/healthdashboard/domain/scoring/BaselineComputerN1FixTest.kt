...
class BaselineComputerN1FixTest {
    ...
    @Test
    fun `computeAdaptiveBaselineRhrBpm returns override when provided`() = runTest {
    ...
    @Test
    fun `computeAdaptiveBaselineRhrBpm correctly computes nadir for 30-day window`() = runTest {
        val dayMidnight = Instant.now()
        ...
        val allHrSamples = (1..30).flatMap { sessionIdx ->
    ...
    @Test
    fun `computeAdaptiveBaselineRhrBpm filters sessions with less than 10 samples`() = runTest {
        val dayMidnight = Instant.now()
        val baselineFromMs = dayMidnight.minus(30, ChronoUnit.DAYS).toEpochMilli()
    ...
    @Test
    fun `computeAdaptiveBaselineRhrBpm returns default when no valid sessions`() = runTest {
        val dayMidnight = Instant.now()
        val baselineFromMs = dayMidnight.minus(30, ChronoUnit.DAYS).toEpochMilli()
    ...
    @Test
    fun `computeAdaptiveBaselineRhrBpm adapts percentile based on sample count`() = runTest {
        val dayMidnight = Instant.now()
        val baselineFromMs = dayMidnight.minus(30, ChronoUnit.DAYS).toEpochMilli()
    ...
    @Test
    fun `query count is O(1) not O(n) for n sessions`() = runTest {
        val dayMidnight = Instant.now()
        val baselineFromMs = dayMidnight.minus(30, ChronoUnit.DAYS).toEpochMilli()
        ...
        val hrSamples = (1..100).flatMap { sessionIdx ->
            (0..149).map { sampleIdx ->
                HeartRateRecordEntity(
                    id = "hr_${sessionIdx}_$sampleIdx",
                    sessionId = "session_$sessionIdx",

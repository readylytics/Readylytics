...
class CircadianConsistencyRepositoryTest {
    ...
    @Test
    fun `emits Calibrating when all sessions are naps under 3 hours`() = runTest {
        val naps = (1..10).map { i ->
            SleepSessionEntity(
                id = "nap$i",
    ...
    @Test
    fun `emits Ready with perfect score when schedule is perfectly consistent`() = runTest {
        val sessions = (0 until 14).map { i ->
    ...
    @Test
    fun `emits Ready with degraded score when schedule is inconsistent`() = runTest {
    ...
    @Test
    fun `Ready result contains correct threshold from preferences`() = runTest {
        val sessions = (0 until 14).map { i ->
            fakeSleepSession(id = "s$i", bedHour = 23, wakeHour = 7, daysAgo = i)
        }

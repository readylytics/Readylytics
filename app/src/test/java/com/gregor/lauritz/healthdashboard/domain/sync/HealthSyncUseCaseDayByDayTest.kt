...
class HealthSyncUseCaseDayByDayTest {
    ...
    @Test
    fun `sync processes each day independently`() = runTest {
    ...
    @Test
    fun `sync completes even if one day fails`() = runTest {
    ...
    @Test
    fun `sync processes days in chronological order (oldest to newest)`() = runTest {
    ...
    @Test
    fun `sync persists step count for each day`() = runTest {
    ...
    @Test
    fun `sync updates last sync timestamp on completion`() = runTest {
    ...
    @Test
    fun `catchUpSync uses 60-day window`() = runTest {
    ...
    @Test
    fun `sync respects device preference for each day`() = runTest {
        val prefs =
            UserPreferences(
    ...
    @Test
    fun `sync prevents concurrent syncs with Mutex`() = runTest {
    ...
    @Test
    fun `sync handles empty Health Connect responses gracefully`() = runTest {
        coEvery {
            hcRepo.readSleepSessions(any(), any())
        } returns emptyList()
        coEvery {
    ...
    @Test
    fun `sync propagates HealthConnect errors`() = runTest {
        coEvery {
            hcRepo.readSleepSessions(any(), any())
    ...
    @Test
    fun `sync logs success and failure counts`() = runTest {
        val result = useCase.sync(windowDays = 3)

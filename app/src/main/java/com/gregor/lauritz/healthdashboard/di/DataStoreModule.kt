            ...
            migrations = listOf(
                object : DataMigration<UserPreferencesProto> {
                    ...
                    override suspend fun migrate(currentData: UserPreferencesProto): UserPreferencesProto {
                        val oldDataStore = androidx.datastore.preferences.core.PreferenceDataStoreFactory.create(
                            produceFile = { oldFile }
                        )
                        val prefs = oldDataStore.data.first()
                        
                        return UserPreferencesProto.newBuilder().apply {
                            prefs[floatPreferencesKey("goal_sleep_hours")]?.let { goalSleepHours = it }
                            prefs[floatPreferencesKey("hrv_baseline_override")]?.let { hrvBaselineOverride = it }
                            prefs[floatPreferencesKey("rhr_baseline_override")]?.let { rhrBaselineOverride = it }
                            ...
                            prefs[booleanPreferencesKey("auto_calculate_max_hr")]?.let { autoCalculateMaxHr = it }
                            ...
                            prefs[floatPreferencesKey("hrv_optimal_threshold")]?.let { hrvOptimalThreshold = it }
                            prefs[floatPreferencesKey("hrv_warning_threshold")]?.let { hrvWarningThreshold = it }
                            prefs[floatPreferencesKey("rhr_optimal_threshold")]?.let { rhrOptimalThreshold = it }
                            prefs[floatPreferencesKey("rhr_warning_threshold")]?.let { rhrWarningThreshold = it }
                            prefs[intPreferencesKey("resting_hr_before_minutes")]?.let { restingHrBeforeMinutes = it }
                            prefs[intPreferencesKey("resting_hr_after_minutes")]?.let { restingHrAfterMinutes = it }
                            prefs[stringPreferencesKey("app_theme")]?.let { 
                                appTheme = AppThemeProto.valueOf("THEME_$it") 
                            }
                            prefs[booleanPreferencesKey("dynamic_color_enabled")]?.let { dynamicColorEnabled = it }
                            ...
                            prefs[intPreferencesKey("consistency_threshold_minutes")]?.let { consistencyThresholdMinutes = it }
                            prefs[intPreferencesKey("consistency_evaluation_days")]?.let { consistencyEvaluationDays = it }
                            prefs[intPreferencesKey("consistency_baseline_days")]?.let { consistencyBaselineDays = it }
                            prefs[floatPreferencesKey("pai_scaling_factor")]?.let { paiScalingFactor = it }
                            prefs[intPreferencesKey("step_goal")]?.let { stepGoal = it }
                            prefs[booleanPreferencesKey("retention_days_enabled")]?.let { retentionDaysEnabled = it }
                            ...
                            prefs[booleanPreferencesKey("collapse_health_connect")]?.let { collapseHealthConnect = it }
                            prefs[booleanPreferencesKey("collapse_baselines_thresholds")]?.let { collapseBaselinesThresholds = it }
                            ...
                            prefs[stringPreferencesKey("circadian_threshold_override_encrypted")]?.let { circadianThresholdOverride = it }
                        }.build()
                    }

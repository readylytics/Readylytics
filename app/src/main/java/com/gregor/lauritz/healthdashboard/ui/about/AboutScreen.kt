...
fun AboutScreen(
    ...
    LazyColumn(
        ...
        item {
            ...
            BulletItem("**Duration (50%)** — how much sleep you got, compared to your goal (default 8 hours, configurable). Includes a small adjustment for how efficient your time in bed was.")
            BulletItem("**Architecture (25%)** — how much of your sleep was deep (slow-wave) sleep and REM. Both matter. Deep sleep is when most physical recovery happens; REM is when your brain processes memory and emotion.")
            BulletItem("**Restoration (25%)** — how rested your estimated recovery-related physiology looks, based on overnight heart rate variability (HRV) and resting heart rate (RHR).")
            
            Spacer(Modifier.height(8.dp))
            BodyText("**Reading the score**")
            BulletItem("**85-100** Excellent. You slept enough, your sleep stages looked balanced, and your autonomic recovery markers were strong.")
            BulletItem("**70-84** Good. Most components are healthy; one is slightly below your norm.")
            BulletItem("**50-69** Fair. Likely a duration shortfall, fragmented sleep, or an off night for HRV/RHR.")
            BulletItem("**Below 50** Poor. Multiple components are below typical. One bad night is rarely meaningful; a streak deserves attention.")

            Spacer(Modifier.height(8.dp))
            BodyText("**Tooltips you'll see**")
            BulletItem("*Deep sleep target 15-25%* of total sleep, with the healthy range narrowing as you age. We do not penalise you if your wearable reports unusual numbers — wearable stage detection is imperfect.")
            BulletItem("*REM target 20-25%* of total sleep, also drifting slightly downward with age.")
            BulletItem("*Sleep efficiency above 85%* is considered good (American Academy of Sleep Medicine consensus).")
        }
        ...
        item {
            ...
            BodyText("You can tighten the threshold to 15 minutes if you're aiming for athlete-grade regularity, or relax it up to 90 minutes.")
        ...
        item {
            ...
            BodyText("The ratio (ATL ÷ CTL) tells us whether you've recently spiked above your recent norm. Around 1.0 means you're training in line with your fitness; substantially above 1.0 means a relative spike.")

            Spacer(Modifier.height(8.dp))
            BodyText("**How we score the ratio**")
            BulletItem("0.8-1.2 → 100 (in your normal range)")
            BulletItem("1.2-1.5 → linearly decays toward 40")
            BulletItem("Above 1.5 → floors at 40 in the current version (we are revisiting this; see \"Honest limitations\" below)")
        ...
        item {
            ...
            Spacer(Modifier.height(8.dp))
            BodyText("The app reads only — it never writes. You can revoke access at any time in Health Connect settings.")
        ...
        item {
            ...
            BulletItem("**Days 1-7 (Calibration).** We collect data. Sleep Score and Circadian Consistency are visible from the first night you log enough sleep, but **HRV-based Restoration is hidden** because a single night of HRV is not interpretable on its own. Readiness uses a placeholder fitness value based on the activity level you tell us during onboarding.")
            BulletItem("**Days 8-21 (Provisional).** We start showing all three scores, but Restoration uses a population-typical estimate of how much your HRV varies day to day. Expect more variability than the mature score.")
            BulletItem("**Days 22-60 (Maturing).** Your personal HRV mean is settled, and we begin blending your personal day-to-day variability with the population estimate. Confidence indicator visible.")
            BulletItem("**Day 60+ (Mature).** All scores use your own personal baselines for both the average and the variability. This is when small day-to-day differences become trustworthy.")
        ...
        item {
            ...
            BulletItem("**HRV (Heart Rate Variability)** — the millisecond-level variation in time between heartbeats. Higher generally indicates better autonomic recovery, *up to a point*.")
            BulletItem("**RMSSD** — the specific HRV measure most apps use. We work with the natural log of RMSSD internally because it behaves better statistically.")
            BulletItem("**RHR (Resting Heart Rate)** — your nocturnal heart rate, averaged across deep portions of sleep.")
            BulletItem("**Deep sleep / Slow-Wave Sleep / N3** — the deepest stage of NREM sleep; growth-hormone release is concentrated here.")
            BulletItem("**REM** — the dreaming stage, important for memory and emotional processing.")
            BulletItem("**TRIMP (Training Impulse)** — a single number summarising the intensity-weighted duration of an exercise session.")
            BulletItem("**ATL / CTL** — short-term and long-term rolling averages of TRIMP. Borrowed from Banister's training-load model and used by most cycling and running apps.")
        }
        ...
        item {
            ...
            BulletItem("**1. Wearable stage detection is imperfect.** Even premium devices misclassify deep and REM sleep on individual nights. We use 7-day rolling values where it matters and downweight architecture if your device's stage signal is unreliable. Architecture differences within ~5% are not meaningful.")
            BulletItem("**2. Population norms are not destiny.** The \"15-25% deep sleep\" range comes from polysomnography studies of healthy adults. Your healthy normal may sit anywhere in that band, and tends to drift lower with age. Our scoring uses age-banded targets so the ceiling shifts with you.")
            BulletItem("**3. The ACWR (Readiness load ratio) is contested.** The methodological literature (Lolli et al. 2019; Impellizzeri et al. 2020, 2021) has shown that the acute-to-chronic ratio is partially a mathematical artifact and is not a validated *causal* injury predictor. We present it as a *load change indicator*, not an injury risk score, and we are evaluating an uncoupled-window version (chronic = days 8-28 prior; acute = last 7) for the next release.")
            BulletItem("**4. Wellness flag.** When your HRV and RHR patterns suggest possible physiological deviation (such as early illness, sustained stress, or functional overreaching), we surface a soft notification. The flag requires two consecutive nights matching the pattern. This is informational only, not medical advice.")
            BulletItem("**5. One night is noise; trends are signal.** Treat any single day's score as a data point, not a verdict. Look at the 7-day trend.")
            BulletItem("**6. This app does not diagnose anything.** If you suspect sleep apnea, a heart condition, an infection, or an injury, see a clinician.")
        }

        item {
            BodyText(
                "*References available on request. Selected primary sources informing the scoring: Buysse 1989 (PSQI); Buysse 2014 (RU-SATED); Ohayon et al. 2004, 2017; Hirshkowitz et al. 2015 (NSF); Boulos et al. 2019 (Lancet Respir Med); Plews et al. 2012, 2013, 2014 (HRV monitoring); Buchheit 2014 (Front Physiol); Le Meur et al. 2013 (parasympathetic hyperactivity); Mishra et al. 2020 (Nat Biomed Eng); Quer et al. 2021 (Nat Med); Phillips et al. 2017 (Sleep Regularity Index); Lunsford-Avery et al. 2018; Windred et al. 2023/2024; Khalsa et al. 2003 (phase-response curve); Banister 1991; Foster 1998; Gabbett 2016; Lolli et al. 2019; Impellizzeri et al. 2020/2021.*",
    ...
    Surface(
        ...
        Column(
            ...
            Row(modifier = Modifier.fillMaxWidth()) {
                Text("Score", Modifier.weight(1.5f), style = MaterialTheme.typography.labelLarge, fontWeight = FontWeight.Bold)
                Text("What it answers", Modifier.weight(3f), style = MaterialTheme.typography.labelLarge, fontWeight = FontWeight.Bold)
                Text("Range", Modifier.weight(1f), style = MaterialTheme.typography.labelLarge, fontWeight = FontWeight.Bold)
            }
...
@Composable
private fun TableRow(col1: String, col2: String, col3: String) {
